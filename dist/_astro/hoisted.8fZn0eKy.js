let F="",J=[],s=null,L=[],H=[],z="info",_=new Date().getFullYear(),S=new Date().getMonth(),W=null;const n=e=>document.getElementById(e),g=e=>typeof e=="number"?e.toLocaleString("sv-SE"):"–",X=e=>e?new Date(e).toLocaleDateString("sv-SE"):"–",q=e=>e?new Date(e).toLocaleString("sv-SE",{dateStyle:"short",timeStyle:"short",hour12:!1}):"–";function c(e,t="success"){const a=n("toast");a.textContent=(t==="success"?"✓ ":"✕ ")+e,a.className=`show ${t}`,clearTimeout(a._t),a._t=setTimeout(()=>a.className="",3e3)}const D={open:{icon:"📬",label:"Inkommen",cls:"s-open"},proposal:{icon:"📤",label:"Offert skickad",cls:"s-proposal"},waiting:{icon:"⏳",label:"Väntar svar",cls:"s-waiting"},confirmed:{icon:"✅",label:"Bekräftad",cls:"s-confirmed"},fakturerad:{icon:"🧾",label:"Fakturerad",cls:"s-fakturerad"},betald:{icon:"💚",label:"Betald",cls:"s-betald"},cancelled:{icon:"❌",label:"Avbruten",cls:"s-cancelled"}};function ie(){let e=sessionStorage.getItem("sk_admin_token");if(!e)try{const t=JSON.parse(localStorage.getItem("sk_admin_mode")||"{}"),a=8*60*60*1e3;t.token&&t.ts&&Date.now()-t.ts<a&&(e=t.token,sessionStorage.setItem("sk_admin_token",e))}catch{}e&&(F=e,Z())}n("login-pw").addEventListener("keydown",e=>e.key==="Enter"&&Y());n("login-btn").addEventListener("click",Y);function Y(){const e=n("login-pw").value.trim();e&&(F=e,sessionStorage.setItem("sk_admin_token",e),localStorage.setItem("sk_admin_mode",JSON.stringify({ts:Date.now(),token:e})),Z())}function Z(){n("login-screen").style.display="none",n("app").style.display="flex",k(),Pe()}n("logout-btn").addEventListener("click",()=>{sessionStorage.removeItem("sk_admin_token"),localStorage.removeItem("sk_admin_mode"),location.reload()});document.querySelectorAll(".nav-btn").forEach(e=>{e.addEventListener("click",()=>{document.querySelectorAll(".nav-btn").forEach(a=>a.classList.remove("active")),document.querySelectorAll(".view").forEach(a=>a.classList.remove("active")),e.classList.add("active");const t=e.dataset.view;n(`view-${t}`).classList.add("active"),t==="calendar"&&O()})});async function y(e,t={}){const a=await fetch(`/.netlify/functions/${e}`,{...t,headers:{"Content-Type":"application/json",Authorization:`Bearer ${F}`,...t.headers||{}}}),r=await a.json();if(a.status===401)throw sessionStorage.removeItem("sk_admin_token"),F="",n("app").style.display="none",n("login-screen").style.display="flex",n("login-error").style.display="block",n("login-error").textContent="Fel lösenord, försök igen.",n("login-pw").value="",n("login-pw").focus(),new Error("Unauthorized");return{ok:a.ok,status:a.status,data:r}}async function k(){const{ok:e,data:t}=await y("admin-carts");if(!e){if(t.error==="Ej behörig"){n("login-error").style.display="block",n("login-error").textContent="Fel lösenord.",n("app").style.display="none",n("login-screen").style.display="flex",sessionStorage.removeItem("sk_admin_token");return}c("Kunde inte hämta varukorgar","error");return}J=t.carts||[];const a=t.summary?.total_unread||0,r=n("global-unread");a>0?(r.style.display="inline",r.textContent=`${a} nya`):r.style.display="none",R(),n("view-calendar").classList.contains("active")&&O()}function oe(){const e=n("filter-search").value.toLowerCase().trim(),t=n("filter-from").value,a=n("filter-to").value;return J.filter(r=>!(e&&!((r.customer_name||"").toLowerCase().includes(e)||(r.id||"").toLowerCase().includes(e))||t&&r.event_date&&r.event_date<t||a&&r.event_date&&r.event_date>a))}["filter-search","filter-from","filter-to"].forEach(e=>{n(e).addEventListener("input",R)});function R(){const e=oe(),t=["open","proposal","waiting","confirmed","fakturerad","betald","cancelled"],a=n("kanban-board");a.innerHTML=t.map(r=>{const i=D[r],d=e.filter(o=>o.status===r);return`
    ...
    </div>`}async function Q(e){s=null,L=[],H=[],z="info";const t=n("detail-overlay"),a=n("detail-panel");t.classList.add("open"),a.classList.add("open"),n("panel-body").innerHTML='<div class="loading-center"><div class="spinner"></div></div>',n("panel-footer").innerHTML="",n("panel-title").textContent="Laddar…";let r,i;try{({ok:r,data:i}=await y(`admin-carts?id=${e}`))}catch(f){c("Kunde inte ladda varukorg: "+f.message,"error"),E();return}if(!r){c("Kunde inte ladda varukorg","error"),E();return}s=i.cart,L=i.messages||[],H=i.audit_log||[],n("panel-title").textContent=`${s.customer_name||"Okänd"} — ${s.id}`;const d=document.getElementById("panel-quick");d&&d.remove();const o=L.filter(f=>f.sender==="customer"&&!f.read_at).length,l=n("chat-unread-badge");o>0?(l.style.display="inline",l.textContent=o):l.style.display="none",ee(),b("info"),T()}function ee(){document.querySelectorAll(".panel-tab").forEach(e=>{e.classList.toggle("active",e.dataset.tab===z),e.onclick=()=>{z=e.dataset.tab,ee(),b(z)}})}function b(e){const t=n("panel-body");e==="info"&&(t.innerHTML=pe()),e==="products"&&(t.innerHTML=fe()),e==="faktura"&&(t.innerHTML=we()),requestAnimationFrame(()=>{document.querySelectorAll(".status-select, .admin-note-area, .prod-input, .chat-textarea").forEach(a=>{a.style.setProperty("background","#221f50","important"),a.style.setProperty("color","#f0eeff","important"),a.style.setProperty("color-scheme","dark","important"),(a.tagName==="SELECT"||a.classList.contains("admin-note-area"))&&a.style.setProperty("border","1px solid rgba(196,181,244,0.25)","important")})}),e==="chat"&&xe(),e==="log"&&(t.innerHTML=he()),e==="info"&&(n("status-select")?.addEventListener("change",async a=>{const r=a.target.value;if(!confirm(`Ändra status till "${D[r]?.label}"?`)){a.target.value=s.status;return}const{ok:i}=await y("cart-update",{method:"POST",body:JSON.stringify({cart_id:s.id,status:r})});i?(s.status=r,c("Status uppdaterad!"),k(),T()):c("Fel vid statusändring","error")}),document.querySelectorAll('input[name="ctype"]').forEach(a=>{a.addEventListener("change",()=>{const r=document.querySelector('input[name="ctype"]:checked')?.value==="b2b",i=document.getElementById("b2b-fields");i&&(i.style.display=r?"grid":"none")})}),n("save-info-btn")?.addEventListener("click",async()=>{const a=n("save-info-btn");a.disabled=!0,a.textContent="⏳ Sparar…";const r={cart_id:s.id,customer_name:n("edit-name")?.value.trim()||null,customer_email:n("edit-email")?.value.trim()||null,customer_phone:n("edit-phone")?.value.trim()||null,event_date:n("edit-date")?.value||null,event_location:n("edit-location")?.value.trim()||null,customer_type:document.querySelector('input[name="ctype"]:checked')?.value||"b2c",customer_address:n("edit-address")?.value.trim()||null,customer_orgnr:n("edit-orgnr")?.value.trim()||null,customer_ref:n("edit-custref")?.value.trim()||null},{ok:i}=await y("cart-update",{method:"POST",body:JSON.stringify(r)});i?(Object.assign(s,{customer_name:r.customer_name,customer_email:r.customer_email,customer_phone:r.customer_phone,event_date:r.event_date,event_location:r.event_location,customer_type:r.customer_type,customer_address:r.customer_address,customer_orgnr:r.customer_orgnr,customer_ref:r.customer_ref}),n("panel-title").textContent=`${s.customer_name||"Okänd"} — ${s.id}`,c("Kunduppgifter sparade!"),k()):c("Kunde inte spara","error"),a.disabled=!1,a.textContent="💾 Spara kunduppgifter"}),n("save-notes-btn")?.addEventListener("click",async()=>{const a=n("admin-notes").value,{ok:r}=await y("cart-update",{method:"POST",body:JSON.stringify({cart_id:s.id,notes_admin:a})});r?(s.notes_admin=a,c("Anteckning sparad!")):c("Kunde inte spara","error")})),e==="chat"&&(n("chat-send-btn")?.addEventListener("click",K),n("chat-input")?.addEventListener("keydown",a=>{a.key==="Enter"&&(a.ctrlKey||a.metaKey)&&K()}))}function C(){const e=document.querySelectorAll("#prod-tbody .prod-row");return Array.from(e).map(t=>{const a=d=>t.querySelector(`[data-field="${d}"]`)?.value?.trim()||"",r=parseInt(a("qty"))||1,i=parseFloat(a("price"))||0;return{id:a("id"),name:a("name"),category:a("category"),qty:r,price:i}}).filter(t=>t.name)}function V(e){const t=document.querySelector(`[data-idx="${e}"]`);if(!t)return;const a=parseInt(t.querySelector('[data-field="qty"]')?.value)||1,r=parseFloat(t.querySelector('[data-field="price"]')?.value)||0,i=document.getElementById(`prod-sum-${e}`);i&&(i.textContent=g(a*r));const o=C().reduce((f,m)=>f+m.price*m.qty,0),l=document.getElementById("prod-tfoot");l&&(l.innerHTML=`<tr class="total-row"><td colspan="3">Totalt (exkl. moms)</td>
    <td style="text-align:right">${g(o)} kr</td><td></td></tr>
  <tr><td colspan="3" style="color:var(--text-faint);font-size:0.8rem">Inkl. 25% moms</td>
    <td style="text-align:right;color:var(--text-muted);font-size:0.85rem">${g(Math.round(o*1.25))} kr</td><td></td></tr>`)}document.addEventListener("input",e=>{if(e.target.matches(".prod-qty, .prod-price")){const t=e.target.dataset.idx;t!==void 0&&V(t)}});function de(e,t){const a=document.querySelector(`[data-field="qty"][data-idx="${e}"]`);if(!a)return;const r=Math.max(1,(parseInt(a.value)||1)+t);a.value=r,V(e)}function le(e){const t=document.querySelector(`[data-idx="${e}"]`)?.closest("tr");t&&(t.remove(),V(-1))}function ce(){const e=document.getElementById("prod-tbody");if(!e)return;const t=Date.now(),a=document.createElement("tr");a.className="prod-row",a.dataset.idx=t,a.innerHTML=`
    <td>
    ...
    </td>`,e.appendChild(a),a.querySelectorAll(".prod-input").forEach(r=>{r.style.setProperty("background","#221f50","important"),r.style.setProperty("color","#f0eeff","important"),r.style.setProperty("color-scheme","dark","important")}),a.querySelector(".prod-name")?.focus()}async function te(){const e=document.getElementById("save-items-btn"),t=document.getElementById("prod-save-status"),a=C(),r=["Tjänster","Tillägg"],i=(s.items||[]).filter(m=>r.includes(m.category)),d=[...a,...i];e&&(e.disabled=!0,e.textContent="⏳ Sparar…");const o=document.getElementById("prod-note")?.value?.trim()||"";o&&d.push({_note:!0,id:"_note",name:o,price:0,qty:1});const l=Math.round(a.filter(m=>!m._note).reduce((m,$)=>m+$.price*$.qty,0)*100),{ok:f}=await y("cart-update",{method:"POST",body:JSON.stringify({cart_id:s.id,items:d,total_excl:l})});f?(s.items=d,s.total_excl=l,t&&(t.textContent="✓ Sparat",t.style.color="var(--green)"),setTimeout(()=>{t&&(t.textContent="")},3e3),c("Produkter uppdaterade!"),R()):(t&&(t.textContent="✗ Fel – kontrollera konsolen",t.style.color="var(--red)"),c("Kunde inte spara","error")),e&&(e.disabled=!1,e.textContent="💾 Spara produkter")}function pe(){const e=s,t=D[e.status];return`
    ...
    <div class="status-row">
      <select class="status-select" id="status-select" style="background:#221f50!important;color:#f0eeff!important;color-scheme:dark;padding-right:2.25rem">
        ${Object.entries(D).map(([a,r])=>`<option value="${a}" ${a===e.status?"selected":""}>${r.icon} ${r.label}</option>`).join("")}
      </select>
    ...
    <div class="info-grid">
      <div class="info-field"><div class="info-field-label">Skapad</div><div class="info-field-value" style="font-size:0.8rem">${q(e.created_at)}</div></div>
      <div class="info-field"><div class="info-field-label">Uppdaterad</div><div class="info-field-value" style="font-size:0.8rem">${q(e.updated_at)}</div></div>
      <div class="info-field"><div class="info-field-label">Kund senast aktiv</div><div class="info-field-value" style="font-size:0.8rem">${q(e.last_read_customer)}</div></div>
  ...
  `}function me(){let e='<option value="">+ Välj produkt att lägga till</option>';return Object.entries(B).forEach(([t,a])=>{a.products?(e+=`<optgroup label="${t}">`,a.products.forEach(r=>{const i=JSON.stringify({id:r.id,name:r.name,price:r.price,category:t}).replace(/"/g,"&quot;");e+=`<option value="${i}">${r.name} — ${g(r.price)} kr</option>`}),e+="</optgroup>"):a.sub&&Object.entries(a.sub).forEach(([r,i])=>{e+=`<optgroup label="${t} — ${r}">`,i.forEach(d=>{const o=JSON.stringify({id:d.id,name:d.name,price:d.price,category:`${t} — ${r}`}).replace(/"/g,"&quot;");e+=`<option value="${o}">${d.name} — ${g(d.price)} kr</option>`}),e+="</optgroup>"})}),e}function ue(e){const t=(e||"").toLowerCase();return t.includes("scen")?{icon:"🎪",color:"var(--blue)"}:t.includes("ljud")?{icon:"🔊",color:"var(--green)"}:t.includes("ljus")?{icon:"💡",color:"var(--amber)"}:t.includes("dj")?{icon:"🎧",color:"var(--brand-lav)"}:t.includes("bild")?{icon:"📽",color:"var(--blue)"}:t.includes("tjänst")||t.includes("leverans")||t.includes("montering")?{icon:"🚚",color:"var(--gray)"}:{icon:"📦",color:"var(--gray)"}}function fe(){const e=s.items||[],t=e.find(o=>o._note),a=["Tjänster","Tillägg"],r=e.filter(o=>!o._note&&!a.includes(o.category)),i=r.reduce((o,l)=>o+(l.price||0)*(l.qty||l.quantity||1),0),d=r.map((o,l)=>{const f=o.qty||o.quantity||1,m=o.price||0,$=f*m,{icon:x,color:h}=ue(o.category),w=o.id||"",u=typeof PROD_DESC<"u"&&PROD_DESC[w]?PROD_DESC[w]:null,p=u?`<div style="font-size:0.78rem;color:var(--text-muted);line-height:1.5;margin-top:4px">${u.desc||""}</div>${u.includes?.length?'<ul style="font-size:0.72rem;color:var(--text-faint);padding-left:14px;margin-top:3px">'+u.includes.map(j=>`<li>${j}</li>`).join("")+"</ul>":""}`:`<div style="font-size:0.78rem;color:var(--text-faint);margin-top:4px">Artno: ${w||"–"} · Kategori: ${o.category||"–"}</div>`;return`<tr data-idx="${l}" class="prod-row">
      <td style="vertical-align:top;padding:8px 6px">
        <div style="display:flex;align-items:flex-start;gap:6px">
          <button onclick="prodTogglePopout(${l})" style="flex-shrink:0;width:28px;height:28px;border-radius:6px;border:1px solid var(--border);background:rgba(0,0,0,.2);color:${h};cursor:pointer;font-size:14px;display:flex;align-items:center;justify-content:center" title="Produktinfo">${x}</button>
          <div style="flex:1;min-width:0">
            ...
            <div id="po-${l}" style="display:none">${p}</div>
          </div>
        </div>
      </td>
      <td style="text-align:center;vertical-align:top;padding-top:10px">
        <div style="display:flex;align-items:center;justify-content:center;gap:2px">
          ...
          <input class="prod-input prod-qty" value="${f}" data-field="qty" data-idx="${l}" style="width:38px;text-align:center;font-size:0.85rem;background:#221f50!important;color:#f0eeff!important;color-scheme:dark;">
          <button class="qty-btn" onclick="prodQty(${l},1)">＋</button>
        </div>
      </td>
      ...
      <td style="text-align:right;vertical-align:top;padding-top:10px;font-weight:600;color:var(--brand-lav);font-size:0.85rem" id="prod-sum-${l}">${g($)} kr</td>
    ...
    ${function(){const o=(s.items||[]).filter(function(p){return["Tjänster","Tillägg"].includes(p.category)}),l=o.find(function(p){return p.id&&p.id.startsWith("lev-")}),f=o.find(function(p){return p.id==="montering"}),m=o.find(function(p){return p.id&&p.id.startsWith("fakturaavgift")}),x=l&&{"lev-standard":"Leverans — Vanlig bil (t&r)|1118|lev-standard","lev-skrymmande":"Leverans — Bil med släp (t&r)|1598|lev-skrymmande","lev-standard-e":"Leverans — Vanlig bil (enkel)|559|lev-standard-e","lev-lastbil":"Leverans — Lastbil (t&r)|2998|lev-lastbil"}[l.id]||"",h=f?"Montering & demontering|"+f.price+"|montering":"",w=m?"Fakturaavgift|"+m.price+"|fakturaavgift":"Fakturaavgift|49|fakturaavgift",u=function(p,j){return p===j?"selected":""};return'<div style="display:grid;grid-template-columns:1fr auto;gap:4px 8px;align-items:center;font-size:0.82rem"><span style="color:var(--text-muted)">Leverans</span><select id="svc-leverans" onchange="prodUpdateSvc()" style="font-size:0.78rem;padding:3px 6px;background:#221f50!important;color:#f0eeff!important;color-scheme:dark;border:1px solid var(--border);border-radius:5px"><option value="" '+(x?"":"selected")+'>— (välj)</option><option value="Leverans — Vanlig bil (t&r)|1118|lev-standard" '+u(x,"Leverans — Vanlig bil (t&r)|1118|lev-standard")+'>Vanlig bil t&amp;r — 1 118 kr</option><option value="Leverans — Bil med släp (t&r)|1598|lev-skrymmande" '+u(x,"Leverans — Bil med släp (t&r)|1598|lev-skrymmande")+'>Bil med släp t&amp;r — 1 598 kr</option><option value="Leverans — Vanlig bil (enkel)|559|lev-standard-e" '+u(x,"Leverans — Vanlig bil (enkel)|559|lev-standard-e")+'>Vanlig bil enkel — 559 kr</option><option value="Leverans — Lastbil (t&r)|2998|lev-lastbil" '+u(x,"Leverans — Lastbil (t&r)|2998|lev-lastbil")+'>Lastbil t&amp;r — 2 998 kr</option><option value="">Ingen leverans</option></select><span style="color:var(--text-muted)">Montering</span><select id="svc-montering" onchange="prodUpdateSvc()" style="font-size:0.78rem;padding:3px 6px;background:#221f50!important;color:#f0eeff!important;color-scheme:dark;border:1px solid var(--border);border-radius:5px"><option value="" '+(h?"":"selected")+'>— (ingen)</option><option value="Montering & demontering|600|montering" '+u(h,"Montering & demontering|600|montering")+'>600 kr (auto)</option><option value="Montering & demontering|1200|montering" '+u(h,"Montering & demontering|1200|montering")+'>1 200 kr (2 tim)</option><option value="Montering & demontering|1800|montering" '+u(h,"Montering & demontering|1800|montering")+'>1 800 kr (3 tim)</option><option value="Montering & demontering|300|montering" '+u(h,"Montering & demontering|300|montering")+'>300 kr (30 min)</option></select></div><div class="section-label" style="margin-top:12px">Fakturaavgift</div><div style="display:grid;grid-template-columns:1fr auto;gap:4px 8px;align-items:center;font-size:0.82rem"><span style="color:var(--text-muted)">Fakturaavgift</span><select id="svc-faktura" onchange="prodUpdateSvc()" style="font-size:0.78rem;padding:3px 6px;background:#221f50!important;color:#f0eeff!important;color-scheme:dark;border:1px solid var(--border);border-radius:5px"><option value="Fakturaavgift|49|fakturaavgift" '+u(w,"Fakturaavgift|49|fakturaavgift")+'>49 kr (standard)</option><option value="Fakturaavgift|29|fakturaavgift" '+u(w,"Fakturaavgift|29|fakturaavgift")+'>29 kr</option><option value="" '+(w?"":"selected")+">Ingen</option></select></div>"}()}

    <div style="border-top:1px solid var(--border);margin-top:14px;padding-top:10px">
      <div style="display:flex;justify-content:space-between;font-size:0.85rem;color:var(--text-muted);margin-bottom:3px"><span>Produkter exkl. moms</span><span>${g(i)} kr</span></div>
      <div style="display:flex;justify-content:space-between;font-size:0.85rem;color:var(--text-muted);margin-bottom:3px"><span>Moms 25%</span><span>${g(Math.round(i*.25))} kr</span></div>
      <div style="display:flex;justify-content:space-between;font-size:1rem;font-weight:700;color:var(--brand-lav)"><span>Inkl. moms</span><span>${g(Math.round(i*1.25))} kr</span></div>
    </div>
  ...
  `}function ve(e){const t=document.getElementById("po-"+e);t&&(t.style.display=t.style.display==="none"?"block":"none")}function ge(e){if(e.value)try{const t=JSON.parse(e.value.replace(/&quot;/g,'"')),a=C();a.push({id:t.id||"",name:t.name||"",price:t.price||0,qty:1,category:t.category||""}),s.items=a,b("products")}catch{c("Fel vid tillägg","error")}}function be(){const e=C();e.push({id:"",name:"",price:0,qty:1,category:""}),s.items=e,b("products")}function ye(){const e=C().filter(a=>!["lev-standard","lev-skrymmande","lev-lastbil","lev-standard-e","montering","fakturaavgift","lev-standard-e","lev-skrymmande-e","lev-lastbil-e"].some(r=>a.id===r||a.id===r+"-e")),t=a=>{const r=document.getElementById(a);if(!r||!r.value)return;const[i,d,o]=r.value.split("|");i&&d&&e.push({id:o,name:i,price:parseInt(d),qty:1,category:"Tjänster"})};t("svc-leverans"),t("svc-montering"),t("svc-faktura"),s.items=e,b("products")}async function ke(){await te(),U(s)}function xe(){const e=n("panel-body"),t=L,a=t.length===0?'<div class="chat-empty">Ingen chatt ännu.</div>':`<div class="chat-messages">${t.map(r=>{const i=r.sender==="admin"?r.read_at?`<span class="read-check">✓✓</span> Läst ${q(r.read_at)}`:"⏳ Inte läst":r.read_at?`Läst ${q(r.read_at)}`:"";return`
          <div class="msg-wrap ${r.sender}">
            <div class="msg-bubble">${r.body.replace(/\n/g,"<br>").replace(/\[([^\]]+)\]\(([^)]+)\)/g,'<a href="$2" style="color:var(--brand-lav)">$1</a>')}</div>
            <div class="msg-meta">
              <span>${r.sender==="admin"?"👤 Du":"🧑 Kund"} · ${q(r.created_at)}</span>
              ${i?`<span>${i}</span>`:""}
            </div>
    ...
    <div style="display:flex;flex-direction:column;gap:0.5rem">
      ${e.map(t=>`
        <div style="background:var(--bg);border-radius:8px;padding:0.75rem;display:flex;gap:1rem;align-items:flex-start">
          <div style="font-size:0.75rem;color:var(--text-faint);white-space:nowrap;min-width:130px">${q(t.created_at)}</div>
          <div>
    ...
    <div style="font-size:0.82rem">
      <div style="display:flex;justify-content:space-between;padding:3px 0;color:var(--text-muted)"><span>Produkter & tjänster exkl. moms</span><span>${g(a)} kr</span></div>
      <div style="display:flex;justify-content:space-between;padding:3px 0;color:var(--text-muted)"><span>Moms 25%</span><span>${g(Math.round(a*.25))} kr</span></div>
      <div style="display:flex;justify-content:space-between;padding:6px 0;font-weight:700;font-size:0.95rem;color:var(--brand-lav);border-top:1px solid var(--border);margin-top:4px"><span>Att betala inkl. moms</span><span>${g(Math.round(a*1.25))} kr</span></div>
    </div>
  ...
  `}async function ae(){const e={cart_id:s.id,invoice_date:document.getElementById("inv-date")?.value||null,invoice_due_date:document.getElementById("inv-due")?.value||null,payment_terms_days:parseInt(document.getElementById("inv-terms")?.value||"5")},{ok:t}=await y("cart-update",{method:"POST",body:JSON.stringify(e)});t?(Object.assign(s,e),c("Fakturauppgifter sparade!"),b("faktura")):c("Kunde inte spara","error")}async function Se(){if(!s.customer_email){c("Kunden saknar e-postadress","error");return}if(!confirm(`Skicka faktura till ${s.customer_email}?`))return;await ae().catch(()=>{});const e=document.querySelector('[onclick="invSendEmail()"]');e&&(e.disabled=!0,e.textContent="⏳ Genererar PDF…");try{const{ok:t,data:a}=await y("invoice-send",{method:"POST",body:JSON.stringify({cart_id:s.id})});t&&a.ok?(s.status="fakturerad",s.invoice_sent_at=new Date().toISOString(),s.invoice_number=a.invoice_number,c(`Faktura ${a.invoice_number} skickad! 📧`),k(),b("faktura"),T()):c(a?.error||"Fel vid utskick","error")}catch(t){c("Nätverksfel: "+t.message,"error")}finally{e&&(e.disabled=!1,e.textContent="📧 Skicka faktura via mail")}}async function re(){if(!confirm("Markera ordern som betald?"))return;const e=new Date().toISOString(),{ok:t}=await y("cart-update",{method:"POST",body:JSON.stringify({cart_id:s.id,status:"betald",invoice_paid_at:e})});t?(s.status="betald",s.invoice_paid_at=e,c("Order markerad som betald! 💚"),k(),b("faktura"),T()):c("Fel vid markering","error")}function T(){const e=s,t=n("panel-footer");t.innerHTML="";const a=(r,i,d)=>{const o=document.createElement("button");o.className=`btn-action ${i}`,o.textContent=r,o.onclick=d,t.appendChild(o)};e.status==="open"?(a("📤 Skapa & skicka offert","btn-success",()=>{b("products")}),a("❌ Avvisa","btn-danger",P)):e.status==="proposal"?(a("✅ Bekräfta order","btn-success",I),a("📦 Justera produkter","",()=>b("products")),a("❌ Avbryt","btn-danger",P)):e.status==="waiting"?(a("✅ Bekräfta order","btn-success",I),a("📩 Påminn kund","",A),a("❌ Avbryt","btn-danger",P)):e.status==="confirmed"?(a("🧾 Skapa faktura","btn-success",()=>b("faktura")),a("📩 Skicka notis","",A)):e.status==="fakturerad"?(a("💚 Markera betald","btn-success",re),a("📩 Påminn om betalning","",A)):e.status==="betald"||e.status!=="cancelled"&&(a("✅ Bekräfta order","btn-success",I),a("📤 Skicka offert","",()=>U(e)),a("❌ Avbryt","btn-danger",P)),e.cart_token&&a("🔗 Kopiera kundlänk","",()=>{const r=`https://scenkonsult.se/order/?cart=${e.id}&token=${e.cart_token}`;navigator.clipboard.writeText(r),c("Kundlänk kopierad!")})}async function I(){if(!confirm("Bekräfta ordern? Status ändras till Orderbekräftelse och TTL tas bort."))return;const{ok:e}=await y("cart-update",{method:"POST",body:JSON.stringify({cart_id:s.id,status:"confirmed"})});e?(s.status="confirmed",c("Order bekräftad! ✅"),k(),b("info"),T()):c("Fel vid bekräftelse","error")}async function P(){if(!confirm("Avbryt ordern? Den sparas i 90 dagar."))return;const{ok:e}=await y("cart-update",{method:"POST",body:JSON.stringify({cart_id:s.id,status:"cancelled"})});e?(s.status="cancelled",c("Order avbruten"),await k(),b("info"),T()):c("Fel vid avbrytning","error")}async function A(){const e=prompt("Meddelande till kund (skickas via e-post):");if(!e?.trim())return;const{ok:t}=await y("cart-message",{method:"POST",body:JSON.stringify({cart_id:s.id,body:e.trim()})});t?(c("Notis skickad till kund! 📩"),L.push({id:Date.now(),cart_id:s.id,sender:"admin",body:e.trim(),read_at:null,created_at:new Date().toISOString()}),z==="chat"&&b("chat")):c("Kunde inte skicka notis","error")}const B={Scen:{products:[{id:"small",name:"Scenpaket Small",price:599},{id:"small-plus",name:"Scenpaket Small+",price:899},{id:"small-plusplus",name:"Scenpaket Small++",price:1199},{id:"medium",name:"Scenpaket Medium",price:1499},{id:"medium-plus",name:"Scenpaket Medium+",price:1799},{id:"medium-plusplus",name:"Scenpaket Medium++",price:2399},{id:"medium-plus-tak",name:"Scenpaket Medium+ inkl. scentak",price:3799},{id:"large",name:"Scenpaket Large",price:2999},{id:"large-plus",name:"Scenpaket Large+",price:3599},{id:"large-plus-tak",name:"Scenpaket Large+ inkl. scentak",price:11999},{id:"xl",name:"Scenpaket XL",price:5399},{id:"xl-plus",name:"Scenpaket XL+",price:7199}]},"Scen tillbehör":{products:[{id:"scentrapp-40-cm-",name:"Scentrapp (40 cm)",price:279},{id:"scentrapp-60-cm-",name:"Scentrapp (60 cm)",price:349},{id:"scenkjol-per-4-m-",name:"Scenkjol (per 4 m)",price:79},{id:"backdrop-3-5-2-5-m",name:"Backdrop 3,5×2,5 m",price:799}]},Ljud:{sub:{Portable:[{id:"portable-small",name:"Portable, Small",price:599},{id:"portable-small-plus",name:"Portable, Small+",price:699},{id:"portable-medium",name:"Portable, Medium",price:799},{id:"portable-medium-plus",name:"Portable, Medium+",price:999},{id:"portable-large",name:"Portable, Large",price:1199}],Event:[{id:"event-small",name:"Event, Small",price:799},{id:"event-small-plus",name:"Event, Small+",price:1199},{id:"event-medium",name:"Event, Medium",price:1599},{id:"event-medium-plus",name:"Event, Medium+",price:2299},{id:"event-large",name:"Event, Large",price:3199}],Music:[{id:"music-small",name:"Music, Small",price:999},{id:"music-small-plus",name:"Music, Small+",price:1299},{id:"music-medium",name:"Music, Medium",price:1799},{id:"music-large",name:"Music, Large",price:2699},{id:"music-xl",name:"Music, XL",price:3599},{id:"music-xxl",name:"Music, XXL",price:5499}],Live:[{id:"live-small",name:"Live, Small",price:599},{id:"live-medium",name:"Live, Medium",price:1199},{id:"live-large",name:"Live, Large",price:1999},{id:"live-xl",name:"Live, XL",price:3199}],Mixers:[{id:"mixer-2-2",name:"Mixerbord 2+2 kanaler",price:159},{id:"mixer-4-2",name:"Mixerbord 4+2 kanaler",price:199},{id:"mixer-4-4",name:"Mixerbord 4+4 kanaler",price:299},{id:"mixer-6-4",name:"Mixerbord 6+4 kanaler",price:349},{id:"mixer-12-4",name:"Mixerbord 12+4 kanaler",price:399},{id:"mixer-16-3",name:"Mixerbord 16+3 kanaler",price:599},{id:"mixer-ilive-t80",name:"Digitalt mixerbord 32+16 kanaler",price:2499}],Mikrofoner:[{id:"handmikrofon",name:"Handmikrofon",price:80},{id:"instrumentmikrofon",name:"Instrumentmikrofon",price:120},{id:"tr-dl-s-handmikrofon",name:"Trådlös handmikrofon",price:400},{id:"tr-dl-st-headset",name:"Trådlöst headset",price:480},{id:"shure-slxd-digitalt-tr-dl-st-system-2-mikrofoner",name:"Shure SLXD trådlöst system (2 mik)",price:1299},{id:"wireless-8ch",name:"Trådlöst mikrofonset, 8 kanaler",price:2999},{id:"mikrofonstativ",name:"Mikrofonstativ",price:60}],"Stativ & kablar":[{id:"h-gtalarstativ-gravity",name:"Högtalarstativ Gravity",price:40},{id:"h-gtalarstativ-k-m",name:"Högtalarstativ K&M",price:50},{id:"h-gtalarstativ-gravity-premium",name:"Högtalarstativ Gravity Premium",price:60},{id:"h-gtalarstativ-gravity-med-vev",name:"Högtalarstativ med vev",price:120},{id:"xlr-kablar",name:"XLR kablar",price:40},{id:"bluetooth-mottagare",name:"Bluetooth-mottagare",price:80}]}},Ljus:{sub:{"Färdiga paket":[{id:"ljus-small",name:"Ljuspaket, Small",price:399},{id:"ljus-small-plus",name:"Ljuspaket, Small+",price:599},{id:"ljus-small-pp",name:"Ljuspaket, Small++",price:799},{id:"ljus-medium",name:"Ljuspaket, Medium",price:1199},{id:"ljus-medium-plus",name:"Ljuspaket, Medium+",price:1299},{id:"ljus-medium-pp",name:"Ljuspaket, Medium++",price:1499},{id:"ljus-large",name:"Ljuspaket, Large",price:1699},{id:"ljus-large-plus",name:"Ljuspaket, Large+",price:2199},{id:"ljus-large-pp",name:"Ljuspaket, Large++",price:2699}],"Lösa effekter":[{id:"led-par",name:"LED PAR (14x8W RGBW)",price:79},{id:"led-par-9",name:"LED PAR (9x10W RGBW)",price:99},{id:"led-par-uv",name:"LED PAR UV",price:119},{id:"led-par-xl",name:"LED PAR XL (60x2W RGBW)",price:149},{id:"led-uppladdningsbar",name:"LED-armatur, uppladdningsbar",price:199}],"Rök & pyro":[{id:"rokmaskin-1500",name:"Rökmaskin (1500W)",price:349},{id:"rokmaskin-650",name:"Rökmaskin (650W)",price:449},{id:"kallgnist",name:"Kallgnistmaskin",price:499},{id:"konfetti",name:"Konfettiavfyrare",price:499}],"Stativ & tross":[{id:"stativ-400",name:"Stativ med T-bar (400 mm)",price:40},{id:"stativ-1200",name:"Stativ med T-bar (1200 mm)",price:40},{id:"stativ-900p",name:"Stativ med T-bar (900 mm, premium)",price:60},{id:"vevstativ-1200",name:"Vevstativ med T-bar (1200 mm)",price:120},{id:"tross-1m",name:"Trosstorn 1,0 m",price:300},{id:"tross-15m",name:"Trosstorn 1,5 m",price:360}]}},"Projektor & skärm":{products:[{id:"projektor-xga",name:"Projektor (XGA)",price:299},{id:"projektor-fhd",name:"Projektor (Full HD)",price:499},{id:"skarm-65",name:'Skärm 65"',price:399},{id:"skarm-75",name:'Skärm 75"',price:799},{id:"trailer-skarm",name:"Trailerupphängd skärm",price:1999}]},"Projektor tillbehör":{products:[{id:"projektorbord",name:"Projektorbord",price:160},{id:"projektorduk-100-",name:'Projektorduk 100"',price:400},{id:"golvstativ-65-75-",name:'Golvstativ 65"/75"',price:240},{id:"hdmi-kabel-5-m",name:"HDMI-kabel 5 m",price:48},{id:"hdmi-kabel-10-m",name:"HDMI-kabel 10 m",price:60},{id:"hdmi-splitter-4-port-",name:"HDMI-splitter (4-port)",price:400},{id:"mediaspelare-4k",name:"Mediaspelare 4K",price:400},{id:"scenmonitor-23-",name:'Scenmonitor 23"',price:480},{id:"usb-clicker-",name:"USB Clicker",price:60}]},"DJ-utrustning":{products:[{id:"dj-controller-numark",name:"DJ-controller Numark Mixstream Pro+",price:799},{id:"dj-controller-denon-go",name:"DJ-controller Denon Prime GO+",price:999},{id:"dj-controller-denon-prime4",name:"DJ-controller Denon Prime 4+",price:1499},{id:"dj-rane-system-one",name:"DJ-system Rane System One",price:1999},{id:"dj-bord",name:"DJ-bord",price:80},{id:"dj-bord2",name:"DJ-bord med paneler",price:200},{id:"scenengineer",name:"Hyr Scenengineer",price:952},{id:"media-editor",name:"Hyr Media-editor",price:952}]},"El & kabel":{products:[{id:"grenuttag-1-5-m",name:"Grenuttag 1,5 m",price:20},{id:"f-rl-ngningskabel-10-m",name:"Förlängningskabel 10 m",price:48},{id:"grenuttag-p-rulle-10-m",name:"Grenuttag på rulle 10 m",price:100},{id:"grenuttag-p-rulle-25-m",name:"Grenuttag på rulle 25 m",price:159}]},Tillägg:{products:[{id:"lev-standard",name:"Leverans & upphämtning (standard bil)",price:1118},{id:"lev-skrymmande",name:"Leverans & upphämtning (bil med släp)",price:1598},{id:"lev-lastbil",name:"Leverans & upphämtning (lätt lastbil)",price:2198},{id:"lev-bakgavel",name:"Leverans & upphämtning (lastbil med bakgavellift)",price:2998},{id:"rigg-teknik",name:"Teknikertjänst på plats (per tim)",price:600},{id:"montering",name:"Montering & demontering (per tim)",price:600},{id:"faktura",name:"Fakturaavgift",price:49}]},"Egen rad":{products:[{id:"custom",name:"Ange benämning och pris →",price:0,custom:!0}]}};n("new-quote-btn").addEventListener("click",()=>G(null));n("quote-close").addEventListener("click",N);n("quote-overlay").addEventListener("click",e=>{e.target===n("quote-overlay")&&N()});let v=[];function G(e){if(v=[],n("qc-name").value=e?.customer_name||"",n("qc-email").value=e?.customer_email||"",n("qc-phone").value=e?.customer_phone||"",n("qc-date").value=e?.event_date||"",n("qc-location").value=e?.event_location||"",n("qc-note").value="",n("quote-error").style.display="none",n("quote-send-btn").disabled=!1,n("quote-send-btn").textContent="✉ Skicka offert",e?.items?.length){e.items.filter(r=>!r._note).forEach(r=>{v.push({id:r.id||"",name:r.name,price:r.price||0,qty:r.qty||1})});const a=e.items.find(r=>r._note);a&&(n("qc-note").value=a.name)}const t=n("qp-cat");t.innerHTML='<option value="">— välj kategori —</option>'+Object.keys(B).map(a=>`<option value="${a}">${a}</option>`).join(""),n("qp-sub-wrap").style.display="none",n("qp-prod").innerHTML='<option value="">— välj kategori —</option>',M(),n("quote-overlay").classList.add("open"),n("qc-name").focus()}function U(e){G(e)}function N(){n("quote-overlay").classList.remove("open")}function $e(){const e=n("qp-cat").value,t=B[e],a=n("qp-sub-wrap"),r=n("qp-prod");if(!t){a.style.display="none",r.innerHTML='<option value="">— välj kategori —</option>';return}if(t.sub){const i=n("qp-sub");i.innerHTML='<option value="">— välj underkategori —</option>'+Object.keys(t.sub).map(d=>`<option value="${d}">${d}</option>`).join(""),a.style.display="",r.innerHTML='<option value="">— välj underkategori —</option>'}else a.style.display="none",ne(t.products)}function _e(){const e=n("qp-cat").value,t=n("qp-sub").value,a=B[e];!a?.sub||!t||ne(a.sub[t]||[])}function ne(e){const t=n("qp-prod");t.innerHTML='<option value="">— välj produkt —</option>'+e.map(a=>{const r=a.custom?"✏ Skriv in benämning och pris själv":`${a.name} — ${a.price.toLocaleString("sv")} kr`;return`<option value="${a.id}" data-price="${a.price}" data-name="${a.name.replace(/"/g,"&quot;")}" data-custom="${a.custom||!1}">${r}</option>`}).join("")}function qe(){const e=n("qp-prod"),t=e.options[e.selectedIndex];if(!t||!t.value)return;if(t.dataset.custom==="true"){v.push({id:"custom-"+Date.now(),name:"",price:0,qty:1,custom:!0}),M(),setTimeout(()=>{const l=document.querySelectorAll(".quote-custom-name");l.length&&l[l.length-1].focus()},50);return}const r=t.dataset.name||t.textContent.split(" — ")[0],i=parseInt(t.dataset.price)||0,d=t.value,o=v.find(l=>l.id===d&&!l.custom);o?o.qty++:v.push({id:d,name:r,price:i,qty:1}),M(),e.selectedIndex=0}function M(){const e=n("quote-prod-tbody"),t=n("quote-prod-tfoot");if(!e)return;e.innerHTML=v.map((r,i)=>{const d=r.custom?`<input class="prod-input quote-custom-name" value="${r.name.replace(/"/g,"&quot;")}" placeholder="Ange benämning..." onchange="quoteEditName(${i},this.value)"
    ...
    <tr data-qidx="${i}">
      ...
      <td style="text-align:right;font-size:0.82rem;font-weight:600;color:var(--brand-lav)">${g(r.price*r.qty)} kr</td>
      <td><button class="qty-btn" style="color:var(--red);border-color:var(--red);width:22px;height:22px" onclick="quoteDelIdx(${i})">×</button></td>
    </tr>`}).join("");const a=v.reduce((r,i)=>r+i.price*i.qty,0);t.innerHTML=v.length?`<tr><td colspan="3" style="font-weight:700;padding-top:0.5rem;font-size:0.82rem">Totalt (exkl. moms)</td>
       <td style="text-align:right;font-weight:700;color:var(--brand-lav);font-size:0.82rem">${g(a)} kr</td><td></td></tr>
       <tr><td colspan="3" style="color:var(--text-faint);font-size:0.72rem">Inkl. 25% moms</td>
       <td style="text-align:right;color:var(--text-muted);font-size:0.72rem">${g(Math.round(a*1.25))} kr</td><td></td></tr>`:""}function Le(e,t){v[e]&&(v[e].qty=Math.max(1,v[e].qty+t),M())}function Me(e,t){v[e]&&(v[e].name=t)}function Ee(e,t){v[e]&&(v[e].price=parseFloat(t)||0,M())}function Te(e){v.splice(e,1),M()}async function je(){const e=n("qc-name").value.trim(),t=n("qc-email").value.trim(),a=n("quote-error");if(a.style.display="none",!e){a.textContent="Namn krävs.",a.style.display="block",n("qc-name").focus();return}if(!t||!t.includes("@")){a.textContent="Giltig e-post krävs.",a.style.display="block",n("qc-email").focus();return}if(v.length===0){a.textContent="Lägg till minst en produkt.",a.style.display="block";return}const r=n("quote-send-btn");r.disabled=!0,r.textContent="⏳ Skickar…";const i={customer:{name:e,email:t,phone:n("qc-phone").value.trim()||null,date:n("qc-date").value||null,location:n("qc-location").value.trim()||null},items:v,note:n("qc-note").value.trim()||null};try{const{ok:d,data:o}=await y("admin-send-quote",{method:"POST",body:JSON.stringify(i)});d?(N(),c(`Offert skickad till ${t}! 🎉`),k()):(a.textContent=o?.error||"Kunde inte skicka offert.",a.style.display="block",r.disabled=!1,r.textContent="✉ Skicka offert")}catch{a.textContent="Nätverksfel — försök igen.",a.style.display="block",r.disabled=!1,r.textContent="✉ Skicka offert"}}n("panel-close").addEventListener("click",E);n("detail-overlay").addEventListener("click",E);document.addEventListener("keydown",e=>e.key==="Escape"&&E());function E(){n("detail-overlay").classList.remove("open"),n("detail-panel").classList.remove("open"),s=null}n("cal-prev").addEventListener("click",()=>{--S<0&&(S=11,_--),O()});n("cal-next").addEventListener("click",()=>{++S>11&&(S=0,_++),O()});n("cal-today").addEventListener("click",()=>{_=new Date().getFullYear(),S=new Date().getMonth(),O()});function O(){const e=["Januari","Februari","Mars","April","Maj","Juni","Juli","Augusti","September","Oktober","November","December"];n("cal-month-label").textContent=`${e[S]} ${_}`;const t=new Date(_,S,1),a=new Date(_,S+1,0),r=new Date;let i=t.getDay()-1;i<0&&(i=6);const d=n("cal-days");d.innerHTML="";const o=Math.ceil((i+a.getDate())/7)*7;for(let l=0;l<o;l++){const f=l-i+1,m=new Date(_,S,f),$=f<1||f>a.getDate(),x=!$&&m.toDateString()===r.toDateString(),h=`${m.getFullYear()}-${String(m.getMonth()+1).padStart(2,"0")}-${String(m.getDate()).padStart(2,"0")}`,w=J.filter(p=>p.event_date===h&&p.status!=="cancelled"),u=document.createElement("div");u.className=`cal-day${$?" other-month":""}${x?" today":""}`,u.innerHTML=`<div class="cal-day-num">${m.getDate()}</div>
      ${w.map(p=>`
        <div class="cal-event s-${p.status}" data-id="${p.id}">
          ${D[p.status]?.icon} ${p.customer_name||p.id}
        </div>`).join("")}`,u.querySelectorAll(".cal-event").forEach(p=>{p.addEventListener("click",j=>{j.stopPropagation(),Q(p.dataset.id)})}),d.appendChild(u)}}n("refresh-btn").addEventListener("click",()=>{k(),c("Uppdaterat!")});function Pe(){clearInterval(W),W=setInterval(k,3e4)}ie();window.prodQty=de;window.prodDelete=le;window.prodAddRow=ce;window.prodSave=te;window.renderTab=b;window.prodTogglePopout=ve;window.prodAddFromCatalog=ge;window.prodAddFreeRow=be;window.prodUpdateSvc=ye;window.prodSaveAndSendOffer=ke;window.invMarkPaid=re;window.invSendEmail=Se;window.invSaveFields=ae;window.openPanel=Q;window.closePanel=E;window.sendMessage=K;window.confirmOrder=I;window.cancelOrder=P;window.notifyCustomer=A;window.quotePickerCat=$e;window.quotePickerSub=_e;window.quotePickerAdd=qe;window.quoteQtyIdx=Le;window.quoteEditName=Me;window.quoteEditPrice=Ee;window.quoteDelIdx=Te;window.sendQuote=je;window.closeQuoteModal=N;window.openQuoteModal=G;window.openQuoteModalForCart=U;
