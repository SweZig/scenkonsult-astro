let I="",K=[],s=null,S=[],N=[],T="info",x=new Date().getFullYear(),k=new Date().getMonth(),V=null;const n=e=>document.getElementById(e),m=e=>typeof e=="number"?e.toLocaleString("sv-SE"):"–",X=e=>e?new Date(e).toLocaleDateString("sv-SE"):"–",h=e=>e?new Date(e).toLocaleString("sv-SE",{dateStyle:"short",timeStyle:"short",hour12:!1}):"–";function l(e,a="success"){const t=n("toast");t.textContent=(a==="success"?"✓ ":"✕ ")+e,t.className=`show ${a}`,clearTimeout(t._t),t._t=setTimeout(()=>t.className="",3e3)}const j={open:{icon:"📬",label:"Inkommen",cls:"s-open"},proposal:{icon:"📤",label:"Offert skickad",cls:"s-proposal"},waiting:{icon:"⏳",label:"Väntar svar",cls:"s-waiting"},confirmed:{icon:"✅",label:"Bekräftad",cls:"s-confirmed"},fakturerad:{icon:"🧾",label:"Fakturerad",cls:"s-fakturerad"},betald:{icon:"💚",label:"Betald",cls:"s-betald"},cancelled:{icon:"❌",label:"Avbruten",cls:"s-cancelled"}};function ie(){let e=sessionStorage.getItem("sk_admin_token");if(!e)try{const a=JSON.parse(localStorage.getItem("sk_admin_mode")||"{}"),t=8*60*60*1e3;a.token&&a.ts&&Date.now()-a.ts<t&&(e=a.token,sessionStorage.setItem("sk_admin_token",e))}catch{}e&&(I=e,Y())}n("login-pw").addEventListener("keydown",e=>e.key==="Enter"&&W());n("login-btn").addEventListener("click",W);function W(){const e=n("login-pw").value.trim();e&&(I=e,sessionStorage.setItem("sk_admin_token",e),localStorage.setItem("sk_admin_mode",JSON.stringify({ts:Date.now(),token:e})),Y())}function Y(){n("login-screen").style.display="none",n("app").style.display="flex",b(),Pe()}n("logout-btn").addEventListener("click",()=>{sessionStorage.removeItem("sk_admin_token"),localStorage.removeItem("sk_admin_mode"),location.reload()});document.querySelectorAll(".nav-btn").forEach(e=>{e.addEventListener("click",()=>{document.querySelectorAll(".nav-btn").forEach(t=>t.classList.remove("active")),document.querySelectorAll(".view").forEach(t=>t.classList.remove("active")),e.classList.add("active");const a=e.dataset.view;n(`view-${a}`).classList.add("active"),a==="calendar"&&z()})});async function f(e,a={}){const t=await fetch(`/.netlify/functions/${e}`,{...a,headers:{"Content-Type":"application/json",Authorization:`Bearer ${I}`,...a.headers||{}}}),r=await t.json();if(t.status===401)throw sessionStorage.removeItem("sk_admin_token"),I="",n("app").style.display="none",n("login-screen").style.display="flex",n("login-error").style.display="block",n("login-error").textContent="Fel lösenord, försök igen.",n("login-pw").value="",n("login-pw").focus(),new Error("Unauthorized");return{ok:t.ok,status:t.status,data:r}}async function b(){const{ok:e,data:a}=await f("admin-carts");if(!e){if(a.error==="Ej behörig"){n("login-error").style.display="block",n("login-error").textContent="Fel lösenord.",n("app").style.display="none",n("login-screen").style.display="flex",sessionStorage.removeItem("sk_admin_token");return}l("Kunde inte hämta varukorgar","error");return}K=a.carts||[];const t=a.summary?.total_unread||0,r=n("global-unread");t>0?(r.style.display="inline",r.textContent=`${t} nya`):r.style.display="none",J(),n("view-calendar").classList.contains("active")&&z()}function oe(){const e=n("filter-search").value.toLowerCase().trim(),a=n("filter-from").value,t=n("filter-to").value;return K.filter(r=>!(e&&!((r.customer_name||"").toLowerCase().includes(e)||(r.id||"").toLowerCase().includes(e))||a&&r.event_date&&r.event_date<a||t&&r.event_date&&r.event_date>t))}["filter-search","filter-from","filter-to"].forEach(e=>{n(e).addEventListener("input",J)});function J(){const e=oe(),a=["open","proposal","waiting","confirmed","fakturerad","betald","cancelled"],t=n("kanban-board");t.innerHTML=a.map(r=>{const i=j[r],o=e.filter(d=>d.status===r);return`
  ...
  `}function me(){let e='<option value="">+ Välj produkt att lägga till</option>';return Object.entries(A).forEach(([a,t])=>{t.products?(e+=`<optgroup label="${a}">`,t.products.forEach(r=>{const i=JSON.stringify({id:r.id,name:r.name,price:r.price,category:a}).replace(/"/g,"&quot;");e+=`<option value="${i}">${r.name} — ${m(r.price)} kr</option>`}),e+="</optgroup>"):t.sub&&Object.entries(t.sub).forEach(([r,i])=>{e+=`<optgroup label="${a} — ${r}">`,i.forEach(o=>{const d=JSON.stringify({id:o.id,name:o.name,price:o.price,category:`${a} — ${r}`}).replace(/"/g,"&quot;");e+=`<option value="${d}">${o.name} — ${m(o.price)} kr</option>`}),e+="</optgroup>"})}),e}function ue(e){const a=(e||"").toLowerCase();return a.includes("scen")?{icon:"🎪",color:"var(--blue)"}:a.includes("ljud")?{icon:"🔊",color:"var(--green)"}:a.includes("ljus")?{icon:"💡",color:"var(--amber)"}:a.includes("dj")?{icon:"🎧",color:"var(--brand-lav)"}:a.includes("bild")?{icon:"📽",color:"var(--blue)"}:a.includes("tjänst")||a.includes("leverans")||a.includes("montering")?{icon:"🚚",color:"var(--gray)"}:{icon:"📦",color:"var(--gray)"}}function fe(){const e=s.items||[],a=e.find(o=>o._note),t=e.filter(o=>!o._note),r=t.reduce((o,d)=>o+(d.price||0)*(d.qty||d.quantity||1),0),i=t.map((o,d)=>{const c=o.qty||o.quantity||1,v=o.price||0,y=c*v,{icon:D,color:B}=ue(o.category),E=o.id||"",w=typeof PROD_DESC<"u"&&PROD_DESC[E]?PROD_DESC[E]:null,$=w?`<div style="font-size:0.78rem;color:var(--text-muted);line-height:1.5;margin-top:4px">${w.desc||""}</div>${w.includes?.length?'<ul style="font-size:0.72rem;color:var(--text-faint);padding-left:14px;margin-top:3px">'+w.includes.map(g=>`<li>${g}</li>`).join("")+"</ul>":""}`:`<div style="font-size:0.78rem;color:var(--text-faint);margin-top:4px">Artno: ${E||"–"} · Kategori: ${o.category||"–"}</div>`;return`<tr data-idx="${d}" class="prod-row">
      ...
      <td style="text-align:right;vertical-align:top;padding-top:10px;font-weight:600;color:var(--brand-lav);font-size:0.85rem" id="prod-sum-${d}">${m(y)} kr</td>
  ...
  `}function ve(e){const a=document.getElementById("po-"+e);a&&(a.style.display=a.style.display==="none"?"block":"none")}function ge(e){if(e.value)try{const a=JSON.parse(e.value.replace(/&quot;/g,'"')),t=P();t.push({id:a.id||"",name:a.name||"",price:a.price||0,qty:1,category:a.category||""}),s.items=t,u("products")}catch{l("Fel vid tillägg","error")}}function be(){const e=P();e.push({id:"",name:"",price:0,qty:1,category:""}),s.items=e,u("products")}function ye(){const e=P().filter(t=>!["lev-standard","lev-skrymmande","lev-lastbil","lev-standard-e","montering","fakturaavgift","lev-standard-e","lev-skrymmande-e","lev-lastbil-e"].some(r=>t.id===r||t.id===r+"-e")),a=t=>{const r=document.getElementById(t);if(!r||!r.value)return;const[i,o,d]=r.value.split("|");i&&o&&e.push({id:d,name:i,price:parseInt(o),qty:1,category:"Tjänster"})};a("svc-leverans"),a("svc-montering"),a("svc-faktura"),s.items=e,u("products")}async function ke(){await ee(),U(s)}function xe(){const e=n("panel-body"),a=S,t=a.length===0?'<div class="chat-empty">Ingen chatt ännu.</div>':`<div class="chat-messages">${a.map(r=>{const i=r.sender==="admin"?r.read_at?`<span class="read-check">✓✓</span> Läst ${h(r.read_at)}`:"⏳ Inte läst":r.read_at?`Läst ${h(r.read_at)}`:"";return`
          <div class="msg-wrap ${r.sender}">
            <div class="msg-bubble">${r.body.replace(/\n/g,"<br>").replace(/\[([^\]]+)\]\(([^)]+)\)/g,'<a href="$2" style="color:var(--brand-lav)">$1</a>')}</div>
            <div class="msg-meta">
              <span>${r.sender==="admin"?"👤 Du":"🧑 Kund"} · ${h(r.created_at)}</span>
              ${i?`<span>${i}</span>`:""}
            </div>
