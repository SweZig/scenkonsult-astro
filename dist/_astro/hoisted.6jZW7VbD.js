let I="",K=[],s=null,S=[],N=[],T="info",h=new Date().getFullYear(),k=new Date().getMonth(),X=null;const n=e=>document.getElementById(e),u=e=>typeof e=="number"?e.toLocaleString("sv-SE"):"–",W=e=>e?new Date(e).toLocaleDateString("sv-SE"):"–",x=e=>e?new Date(e).toLocaleString("sv-SE",{dateStyle:"short",timeStyle:"short",hour12:!1}):"–";function l(e,a="success"){const t=n("toast");t.textContent=(a==="success"?"✓ ":"✕ ")+e,t.className=`show ${a}`,clearTimeout(t._t),t._t=setTimeout(()=>t.className="",3e3)}const j={open:{icon:"📬",label:"Inkommen",cls:"s-open"},proposal:{icon:"📤",label:"Offert skickad",cls:"s-proposal"},waiting:{icon:"⏳",label:"Väntar svar",cls:"s-waiting"},confirmed:{icon:"✅",label:"Bekräftad",cls:"s-confirmed"},fakturerad:{icon:"🧾",label:"Fakturerad",cls:"s-fakturerad"},betald:{icon:"💚",label:"Betald",cls:"s-betald"},cancelled:{icon:"❌",label:"Avbruten",cls:"s-cancelled"}};function ie(){let e=sessionStorage.getItem("sk_admin_token");if(!e)try{const a=JSON.parse(localStorage.getItem("sk_admin_mode")||"{}"),t=8*60*60*1e3;a.token&&a.ts&&Date.now()-a.ts<t&&(e=a.token,sessionStorage.setItem("sk_admin_token",e))}catch{}e&&(I=e,Y())}n("login-pw").addEventListener("keydown",e=>e.key==="Enter"&&V());n("login-btn").addEventListener("click",V);function V(){const e=n("login-pw").value.trim();e&&(I=e,sessionStorage.setItem("sk_admin_token",e),localStorage.setItem("sk_admin_mode",JSON.stringify({ts:Date.now(),token:e})),Y())}function Y(){n("login-screen").style.display="none",n("app").style.display="flex",b(),Pe()}n("logout-btn").addEventListener("click",()=>{sessionStorage.removeItem("sk_admin_token"),localStorage.removeItem("sk_admin_mode"),location.reload()});document.querySelectorAll(".nav-btn").forEach(e=>{e.addEventListener("click",()=>{document.querySelectorAll(".nav-btn").forEach(t=>t.classList.remove("active")),document.querySelectorAll(".view").forEach(t=>t.classList.remove("active")),e.classList.add("active");const a=e.dataset.view;n(`view-${a}`).classList.add("active"),a==="calendar"&&z()})});async function f(e,a={}){const t=await fetch(`/.netlify/functions/${e}`,{...a,headers:{"Content-Type":"application/json",Authorization:`Bearer ${I}`,...a.headers||{}}}),r=await t.json();if(t.status===401)throw sessionStorage.removeItem("sk_admin_token"),I="",n("app").style.display="none",n("login-screen").style.display="flex",n("login-error").style.display="block",n("login-error").textContent="Fel lösenord, försök igen.",n("login-pw").value="",n("login-pw").focus(),new Error("Unauthorized");return{ok:t.ok,status:t.status,data:r}}async function b(){const{ok:e,data:a}=await f("admin-carts");if(!e){if(a.error==="Ej behörig"){n("login-error").style.display="block",n("login-error").textContent="Fel lösenord.",n("app").style.display="none",n("login-screen").style.display="flex",sessionStorage.removeItem("sk_admin_token");return}l("Kunde inte hämta varukorgar","error");return}K=a.carts||[];const t=a.summary?.total_unread||0,r=n("global-unread");t>0?(r.style.display="inline",r.textContent=`${t} nya`):r.style.display="none",J(),n("view-calendar").classList.contains("active")&&z()}function oe(){const e=n("filter-search").value.toLowerCase().trim(),a=n("filter-from").value,t=n("filter-to").value;return K.filter(r=>!(e&&!((r.customer_name||"").toLowerCase().includes(e)||(r.id||"").toLowerCase().includes(e))||a&&r.event_date&&r.event_date<a||t&&r.event_date&&r.event_date>t))}["filter-search","filter-from","filter-to"].forEach(e=>{n(e).addEventListener("input",J)});function J(){const e=oe(),a=["open","proposal","waiting","confirmed","fakturerad","betald","cancelled"],t=n("kanban-board");t.innerHTML=a.map(r=>{const i=j[r],o=e.filter(d=>d.status===r);return`
    ...
    </div>`}async function R(e){s=null,S=[],N=[],T="info";const a=n("detail-overlay"),t=n("detail-panel");a.classList.add("open"),t.classList.add("open"),n("panel-body").innerHTML='<div class="loading-center"><div class="spinner"></div></div>',n("panel-footer").innerHTML="",n("panel-title").textContent="Laddar…";let r,i;try{({ok:r,data:i}=await f(`admin-carts?id=${e}`))}catch(v){l("Kunde inte ladda varukorg: "+v.message,"error"),q();return}if(!r){l("Kunde inte ladda varukorg","error"),q();return}s=i.cart,S=i.messages||[],N=i.audit_log||[],n("panel-title").textContent=`${s.customer_name||"Okänd"} — ${s.id}`;const o=document.getElementById("panel-quick");o&&o.remove();const d=S.filter(v=>v.sender==="customer"&&!v.read_at).length,c=n("chat-unread-badge");d>0?(c.style.display="inline",c.textContent=d):c.style.display="none",Z(),m("info"),L()}function Z(){document.querySelectorAll(".panel-tab").forEach(e=>{e.classList.toggle("active",e.dataset.tab===T),e.onclick=()=>{T=e.dataset.tab,Z(),m(T)}})}function m(e){const a=n("panel-body");e==="info"&&(a.innerHTML=pe()),e==="products"&&(a.innerHTML=fe()),e==="faktura"&&(a.innerHTML=we()),requestAnimationFrame(()=>{document.querySelectorAll(".status-select, .admin-note-area, .prod-input, .chat-textarea").forEach(t=>{t.style.setProperty("background","#221f50","important"),t.style.setProperty("color","#f0eeff","important"),t.style.setProperty("color-scheme","dark","important"),(t.tagName==="SELECT"||t.classList.contains("admin-note-area"))&&t.style.setProperty("border","1px solid rgba(196,181,244,0.25)","important")})}),e==="chat"&&he(),e==="log"&&(a.innerHTML=xe()),e==="info"&&(n("status-select")?.addEventListener("change",async t=>{const r=t.target.value;if(!confirm(`Ändra status till "${j[r]?.label}"?`)){t.target.value=s.status;return}const{ok:i}=await f("cart-update",{method:"POST",body:JSON.stringify({cart_id:s.id,status:r})});i?(s.status=r,l("Status uppdaterad!"),b(),L()):l("Fel vid statusändring","error")}),document.querySelectorAll('input[name="ctype"]').forEach(t=>{t.addEventListener("change",()=>{const r=document.querySelector('input[name="ctype"]:checked')?.value==="b2b",i=document.getElementById("b2b-fields");i&&(i.style.display=r?"grid":"none")})}),n("save-info-btn")?.addEventListener("click",async()=>{const t=n("save-info-btn");t.disabled=!0,t.textContent="⏳ Sparar…";const r={cart_id:s.id,customer_name:n("edit-name")?.value.trim()||null,customer_email:n("edit-email")?.value.trim()||null,customer_phone:n("edit-phone")?.value.trim()||null,event_date:n("edit-date")?.value||null,event_location:n("edit-location")?.value.trim()||null,customer_type:document.querySelector('input[name="ctype"]:checked')?.value||"b2c",customer_address:n("edit-address")?.value.trim()||null,customer_orgnr:n("edit-orgnr")?.value.trim()||null,customer_ref:n("edit-custref")?.value.trim()||null},{ok:i}=await f("cart-update",{method:"POST",body:JSON.stringify(r)});i?(Object.assign(s,{customer_name:r.customer_name,customer_email:r.customer_email,customer_phone:r.customer_phone,event_date:r.event_date,event_location:r.event_location,customer_type:r.customer_type,customer_address:r.customer_address,customer_orgnr:r.customer_orgnr,customer_ref:r.customer_ref}),n("panel-title").textContent=`${s.customer_name||"Okänd"} — ${s.id}`,l("Kunduppgifter sparade!"),b()):l("Kunde inte spara","error"),t.disabled=!1,t.textContent="💾 Spara kunduppgifter"}),n("save-notes-btn")?.addEventListener("click",async()=>{const t=n("admin-notes").value,{ok:r}=await f("cart-update",{method:"POST",body:JSON.stringify({cart_id:s.id,notes_admin:t})});r?(s.notes_admin=t,l("Anteckning sparad!")):l("Kunde inte spara","error")})),e==="chat"&&(n("chat-send-btn")?.addEventListener("click",H),n("chat-input")?.addEventListener("keydown",t=>{t.key==="Enter"&&(t.ctrlKey||t.metaKey)&&H()}))}function P(){const e=document.querySelectorAll("#prod-tbody .prod-row");return Array.from(e).map(a=>{const t=o=>a.querySelector(`[data-field="${o}"]`)?.value?.trim()||"",r=parseInt(t("qty"))||1,i=parseFloat(t("price"))||0;return{id:t("id"),name:t("name"),category:t("category"),qty:r,price:i}}).filter(a=>a.name)}function Q(e){const a=document.querySelector(`[data-idx="${e}"]`);if(!a)return;const t=parseInt(a.querySelector('[data-field="qty"]')?.value)||1,r=parseFloat(a.querySelector('[data-field="price"]')?.value)||0,i=document.getElementById(`prod-sum-${e}`);i&&(i.textContent=u(t*r));const d=P().reduce((v,y)=>v+y.price*y.qty,0),c=document.getElementById("prod-tfoot");c&&(c.innerHTML=`<tr class="total-row"><td colspan="3">Totalt (exkl. moms)</td>
    <td style="text-align:right">${u(d)} kr</td><td></td></tr>
  <tr><td colspan="3" style="color:var(--text-faint);font-size:0.8rem">Inkl. 25% moms</td>
    <td style="text-align:right;color:var(--text-muted);font-size:0.85rem">${u(Math.round(d*1.25))} kr</td><td></td></tr>`)}document.addEventListener("input",e=>{if(e.target.matches(".prod-qty, .prod-price")){const a=e.target.dataset.idx;a!==void 0&&Q(a)}});function se(e,a){const t=document.querySelector(`[data-field="qty"][data-idx="${e}"]`);if(!t)return;const r=Math.max(1,(parseInt(t.value)||1)+a);t.value=r,Q(e)}function le(e){const a=document.querySelector(`[data-idx="${e}"]`)?.closest("tr");a&&(a.remove(),Q(-1))}function ce(){const e=document.getElementById("prod-tbody");if(!e)return;const a=Date.now(),t=document.createElement("tr");t.className="prod-row",t.dataset.idx=a,t.innerHTML=`
    <td>
    ...
    </td>`,e.appendChild(t),t.querySelectorAll(".prod-input").forEach(r=>{r.style.setProperty("background","#221f50","important"),r.style.setProperty("color","#f0eeff","important"),r.style.setProperty("color-scheme","dark","important")}),t.querySelector(".prod-name")?.focus()}async function ee(){const e=document.getElementById("save-items-btn"),a=document.getElementById("prod-save-status"),t=P();e&&(e.disabled=!0,e.textContent="⏳ Sparar…");const r=document.getElementById("prod-note")?.value?.trim()||"";r&&t.push({_note:!0,id:"_note",name:r,price:0,qty:1});const i=Math.round(t.filter(d=>!d._note).reduce((d,c)=>d+c.price*c.qty,0)*100),{ok:o}=await f("cart-update",{method:"POST",body:JSON.stringify({cart_id:s.id,items:t,total_excl:i})});o?(s.items=t,s.total_excl=i,a&&(a.textContent="✓ Sparat",a.style.color="var(--green)"),setTimeout(()=>{a&&(a.textContent="")},3e3),l("Produkter uppdaterade!"),J()):(a&&(a.textContent="✗ Fel – kontrollera konsolen",a.style.color="var(--red)"),l("Kunde inte spara","error")),e&&(e.disabled=!1,e.textContent="💾 Spara produkter")}function pe(){const e=s,a=j[e.status];return`
    ...
    <div class="status-row">
      <select class="status-select" id="status-select" style="background:#221f50!important;color:#f0eeff!important;color-scheme:dark;padding-right:2.25rem">
        ${Object.entries(j).map(([t,r])=>`<option value="${t}" ${t===e.status?"selected":""}>${r.icon} ${r.label}</option>`).join("")}
      </select>
  ...
  `}function ue(){let e='<option value="">+ Välj produkt att lägga till</option>';return Object.entries(A).forEach(([a,t])=>{t.products?(e+=`<optgroup label="${a}">`,t.products.forEach(r=>{const i=JSON.stringify({id:r.id,name:r.name,price:r.price,category:a}).replace(/"/g,"&quot;");e+=`<option value="${i}">${r.name} — ${u(r.price)} kr</option>`}),e+="</optgroup>"):t.sub&&Object.entries(t.sub).forEach(([r,i])=>{e+=`<optgroup label="${a} — ${r}">`,i.forEach(o=>{const d=JSON.stringify({id:o.id,name:o.name,price:o.price,category:`${a} — ${r}`}).replace(/"/g,"&quot;");e+=`<option value="${d}">${o.name} — ${u(o.price)} kr</option>`}),e+="</optgroup>"})}),e}function me(e){const a=(e||"").toLowerCase();return a.includes("scen")?{icon:"🎪",color:"var(--blue)"}:a.includes("ljud")?{icon:"🔊",color:"var(--green)"}:a.includes("ljus")?{icon:"💡",color:"var(--amber)"}:a.includes("dj")?{icon:"🎧",color:"var(--brand-lav)"}:a.includes("bild")?{icon:"📽",color:"var(--blue)"}:a.includes("tjänst")||a.includes("leverans")||a.includes("montering")?{icon:"🚚",color:"var(--gray)"}:{icon:"📦",color:"var(--gray)"}}function fe(){const e=s.items||[],a=e.find(o=>o._note),t=e.filter(o=>!o._note),r=t.reduce((o,d)=>o+(d.price||0)*(d.qty||d.quantity||1),0),i=t.map((o,d)=>{const c=o.qty||o.quantity||1,v=o.price||0,y=c*v,{icon:D,color:B}=me(o.category),E=o.id||"",w=typeof PROD_DESC<"u"&&PROD_DESC[E]?PROD_DESC[E]:null,$=w?`<div style="font-size:0.78rem;color:var(--text-muted);line-height:1.5;margin-top:4px">${w.desc||""}</div>${w.includes?.length?'<ul style="font-size:0.72rem;color:var(--text-faint);padding-left:14px;margin-top:3px">'+w.includes.map(g=>`<li>${g}</li>`).join("")+"</ul>":""}`:`<div style="font-size:0.78rem;color:var(--text-faint);margin-top:4px">Artno: ${E||"–"} · Kategori: ${o.category||"–"}</div>`;return`<tr data-idx="${d}" class="prod-row">
      <td style="vertical-align:top;padding:8px 6px">
        <div style="display:flex;align-items:flex-start;gap:6px">
          ...
          <div style="flex:1;min-width:0">
            ...
            <input type="hidden" data-field="category" data-idx="${d}" value="${(o.category||"").replace(/"/g,"&quot;")}">
            <input type="hidden" data-field="id" data-idx="${d}" value="${(o.id||"").replace(/"/g,"&quot;")}">
      ...
      <td style="text-align:right;vertical-align:top;padding-top:10px">
        <input class="prod-input prod-price" value="${v}" data-field="price" data-idx="${d}" style="width:78px;text-align:right;font-size:0.85rem;background:#221f50!important;color:#f0eeff!important;color-scheme:dark;">
      </td>
      <td style="text-align:right;vertical-align:top;padding-top:10px;font-weight:600;color:var(--brand-lav);font-size:0.85rem" id="prod-sum-${d}">${u(y)} kr</td>
    ...
    <div style="border-top:1px solid var(--border);margin-top:14px;padding-top:10px">
      <div style="display:flex;justify-content:space-between;font-size:0.85rem;color:var(--text-muted);margin-bottom:3px"><span>Produkter exkl. moms</span><span>${u(r)} kr</span></div>
      <div style="display:flex;justify-content:space-between;font-size:0.85rem;color:var(--text-muted);margin-bottom:3px"><span>Moms 25%</span><span>${u(Math.round(r*.25))} kr</span></div>
      <div style="display:flex;justify-content:space-between;font-size:1rem;font-weight:700;color:var(--brand-lav)"><span>Inkl. moms</span><span>${u(Math.round(r*1.25))} kr</span></div>
    </div>
  ...
  `}function ve(e){const a=document.getElementById("po-"+e);a&&(a.style.display=a.style.display==="none"?"block":"none")}function ge(e){if(e.value)try{const a=JSON.parse(e.value.replace(/&quot;/g,'"')),t=P();t.push({id:a.id||"",name:a.name||"",price:a.price||0,qty:1,category:a.category||""}),s.items=t,m("products")}catch{l("Fel vid tillägg","error")}}function be(){const e=P();e.push({id:"",name:"",price:0,qty:1,category:""}),s.items=e,m("products")}function ye(){const e=P().filter(t=>!["lev-standard","lev-skrymmande","lev-lastbil","lev-standard-e","montering","fakturaavgift","lev-standard-e","lev-skrymmande-e","lev-lastbil-e"].some(r=>t.id===r||t.id===r+"-e")),a=t=>{const r=document.getElementById(t);if(!r||!r.value)return;const[i,o,d]=r.value.split("|");i&&o&&e.push({id:d,name:i,price:parseInt(o),qty:1,category:"Tjänster"})};a("svc-leverans"),a("svc-montering"),a("svc-faktura"),s.items=e,m("products")}async function ke(){await ee(),U(s)}function he(){const e=n("panel-body"),a=S,t=a.length===0?'<div class="chat-empty">Ingen chatt ännu.</div>':`<div class="chat-messages">${a.map(r=>{const i=r.sender==="admin"?r.read_at?`<span class="read-check">✓✓</span> Läst ${x(r.read_at)}`:"⏳ Inte läst":r.read_at?`Läst ${x(r.read_at)}`:"";return`
    ...
    </div>`,setTimeout(()=>{const r=e.querySelector(".chat-messages");r&&(r.scrollTop=r.scrollHeight),e.scrollTop=e.scrollHeight},50)}async function H(){const e=n("chat-input"),a=n("chat-send-btn"),t=e?.value.trim();if(!t)return;a.disabled=!0,a.textContent="Skickar…";const{ok:r,data:i}=await f("cart-message",{method:"POST",body:JSON.stringify({cart_id:s.id,body:t})});r?(S.push({id:i.message?.id||Date.now(),cart_id:s.id,sender:"admin",body:t,read_at:null,created_at:new Date().toISOString()}),e.value="",m("chat"),l("Meddelande skickat!"),b()):(l(i.error||"Kunde inte skicka","error"),a.disabled=!1,a.textContent="Skicka till kund →")}function xe(){const e=N;return e.length===0?'<div class="empty-col" style="padding:3rem">Ingen logg ännu.</div>':`
    ...
    </div>`}function we(){const e=s,t=(e.items||[]).filter(d=>!d._note).reduce((d,c)=>d+(c.price||0)*(c.qty||1),0),r=e.invoice_number||"— (ej skapad)",i=!!e.invoice_paid_at,o=!!e.invoice_sent_at;return`
    ...
    <div style="font-size:0.82rem">
      <div style="display:flex;justify-content:space-between;padding:3px 0;color:var(--text-muted)"><span>Produkter & tjänster exkl. moms</span><span>${u(t)} kr</span></div>
      <div style="display:flex;justify-content:space-between;padding:3px 0;color:var(--text-muted)"><span>Moms 25%</span><span>${u(Math.round(t*.25))} kr</span></div>
      <div style="display:flex;justify-content:space-between;padding:6px 0;font-weight:700;font-size:0.95rem;color:var(--brand-lav);border-top:1px solid var(--border);margin-top:4px"><span>Att betala inkl. moms</span><span>${u(Math.round(t*1.25))} kr</span></div>
    </div>
  ...
  `}async function te(){const e={cart_id:s.id,invoice_date:document.getElementById("inv-date")?.value||null,invoice_due_date:document.getElementById("inv-due")?.value||null,payment_terms_days:parseInt(document.getElementById("inv-terms")?.value||"5")},{ok:a}=await f("cart-update",{method:"POST",body:JSON.stringify(e)});a?(Object.assign(s,e),l("Fakturauppgifter sparade!"),m("faktura")):l("Kunde inte spara","error")}async function $e(){if(!s.customer_email){l("Kunden saknar e-postadress","error");return}if(!confirm(`Skicka faktura till ${s.customer_email}?`))return;await te().catch(()=>{});const e=document.querySelector('[onclick="invSendEmail()"]');e&&(e.disabled=!0,e.textContent="⏳ Genererar PDF…");try{const{ok:a,data:t}=await f("invoice-send",{method:"POST",body:JSON.stringify({cart_id:s.id})});a&&t.ok?(s.status="fakturerad",s.invoice_sent_at=new Date().toISOString(),s.invoice_number=t.invoice_number,l(`Faktura ${t.invoice_number} skickad! 📧`),b(),m("faktura"),L()):l(t?.error||"Fel vid utskick","error")}catch(a){l("Nätverksfel: "+a.message,"error")}finally{e&&(e.disabled=!1,e.textContent="📧 Skicka faktura via mail")}}async function ae(){if(!confirm("Markera ordern som betald?"))return;const e=new Date().toISOString(),{ok:a}=await f("cart-update",{method:"POST",body:JSON.stringify({cart_id:s.id,status:"betald",invoice_paid_at:e})});a?(s.status="betald",s.invoice_paid_at=e,l("Order markerad som betald! 💚"),b(),m("faktura"),L()):l("Fel vid markering","error")}function L(){const e=s,a=n("panel-footer");a.innerHTML="";const t=(r,i,o)=>{const d=document.createElement("button");d.className=`btn-action ${i}`,d.textContent=r,d.onclick=o,a.appendChild(d)};e.status==="open"?(t("📤 Skapa & skicka offert","btn-success",()=>{m("products")}),t("❌ Avvisa","btn-danger",M)):e.status==="proposal"?(t("✅ Bekräfta order","btn-success",O),t("📦 Justera produkter","",()=>m("products")),t("❌ Avbryt","btn-danger",M)):e.status==="waiting"?(t("✅ Bekräfta order","btn-success",O),t("📩 Påminn kund","",C),t("❌ Avbryt","btn-danger",M)):e.status==="confirmed"?(t("🧾 Skapa faktura","btn-success",()=>m("faktura")),t("📩 Skicka notis","",C)):e.status==="fakturerad"?(t("💚 Markera betald","btn-success",ae),t("📩 Påminn om betalning","",C)):e.status==="betald"||e.status!=="cancelled"&&(t("✅ Bekräfta order","btn-success",O),t("📤 Skicka offert","",()=>U(e)),t("❌ Avbryt","btn-danger",M)),e.cart_token&&t("🔗 Kopiera kundlänk","",()=>{const r=`https://scenkonsult.se/order/?cart=${e.id}&token=${e.cart_token}`;navigator.clipboard.writeText(r),l("Kundlänk kopierad!")})}async function O(){if(!confirm("Bekräfta ordern? Status ändras till Orderbekräftelse och TTL tas bort."))return;const{ok:e}=await f("cart-update",{method:"POST",body:JSON.stringify({cart_id:s.id,status:"confirmed"})});e?(s.status="confirmed",l("Order bekräftad! ✅"),b(),m("info"),L()):l("Fel vid bekräftelse","error")}async function M(){if(!confirm("Avbryt ordern? Den sparas i 90 dagar."))return;const{ok:e}=await f("cart-update",{method:"POST",body:JSON.stringify({cart_id:s.id,status:"cancelled"})});e?(s.status="cancelled",l("Order avbruten"),await b(),m("info"),L()):l("Fel vid avbrytning","error")}async function C(){const e=prompt("Meddelande till kund (skickas via e-post):");if(!e?.trim())return;const{ok:a}=await f("cart-message",{method:"POST",body:JSON.stringify({cart_id:s.id,body:e.trim()})});a?(l("Notis skickad till kund! 📩"),S.push({id:Date.now(),cart_id:s.id,sender:"admin",body:e.trim(),read_at:null,created_at:new Date().toISOString()}),T==="chat"&&m("chat")):l("Kunde inte skicka notis","error")}const A={Scen:{products:[{id:"small",name:"Scenpaket Small",price:599},{id:"small-plus",name:"Scenpaket Small+",price:899},{id:"small-plusplus",name:"Scenpaket Small++",price:1199},{id:"medium",name:"Scenpaket Medium",price:1499},{id:"medium-plus",name:"Scenpaket Medium+",price:1799},{id:"medium-plusplus",name:"Scenpaket Medium++",price:2399},{id:"medium-plus-tak",name:"Scenpaket Medium+ inkl. scentak",price:3799},{id:"large",name:"Scenpaket Large",price:2999},{id:"large-plus",name:"Scenpaket Large+",price:3599},{id:"large-plus-tak",name:"Scenpaket Large+ inkl. scentak",price:11999},{id:"xl",name:"Scenpaket XL",price:5399},{id:"xl-plus",name:"Scenpaket XL+",price:7199}]},"Scen tillbehör":{products:[{id:"scentrapp-40-cm-",name:"Scentrapp (40 cm)",price:279},{id:"scentrapp-60-cm-",name:"Scentrapp (60 cm)",price:349},{id:"scenkjol-per-4-m-",name:"Scenkjol (per 4 m)",price:79},{id:"backdrop-3-5-2-5-m",name:"Backdrop 3,5×2,5 m",price:799}]},Ljud:{sub:{Portable:[{id:"portable-small",name:"Portable, Small",price:599},{id:"portable-small-plus",name:"Portable, Small+",price:699},{id:"portable-medium",name:"Portable, Medium",price:799},{id:"portable-medium-plus",name:"Portable, Medium+",price:999},{id:"portable-large",name:"Portable, Large",price:1199}],Event:[{id:"event-small",name:"Event, Small",price:799},{id:"event-small-plus",name:"Event, Small+",price:1199},{id:"event-medium",name:"Event, Medium",price:1599},{id:"event-medium-plus",name:"Event, Medium+",price:2299},{id:"event-large",name:"Event, Large",price:3199}],Music:[{id:"music-small",name:"Music, Small",price:999},{id:"music-small-plus",name:"Music, Small+",price:1299},{id:"music-medium",name:"Music, Medium",price:1799},{id:"music-large",name:"Music, Large",price:2699},{id:"music-xl",name:"Music, XL",price:3599},{id:"music-xxl",name:"Music, XXL",price:5499}],Live:[{id:"live-small",name:"Live, Small",price:599},{id:"live-medium",name:"Live, Medium",price:1199},{id:"live-large",name:"Live, Large",price:1999},{id:"live-xl",name:"Live, XL",price:3199}],Mixers:[{id:"mixer-2-2",name:"Mixerbord 2+2 kanaler",price:159},{id:"mixer-4-2",name:"Mixerbord 4+2 kanaler",price:199},{id:"mixer-4-4",name:"Mixerbord 4+4 kanaler",price:299},{id:"mixer-6-4",name:"Mixerbord 6+4 kanaler",price:349},{id:"mixer-12-4",name:"Mixerbord 12+4 kanaler",price:399},{id:"mixer-16-3",name:"Mixerbord 16+3 kanaler",price:599},{id:"mixer-ilive-t80",name:"Digitalt mixerbord 32+16 kanaler",price:2499}],Mikrofoner:[{id:"handmikrofon",name:"Handmikrofon",price:80},{id:"instrumentmikrofon",name:"Instrumentmikrofon",price:120},{id:"tr-dl-s-handmikrofon",name:"Trådlös handmikrofon",price:400},{id:"tr-dl-st-headset",name:"Trådlöst headset",price:480},{id:"shure-slxd-digitalt-tr-dl-st-system-2-mikrofoner",name:"Shure SLXD trådlöst system (2 mik)",price:1299},{id:"wireless-8ch",name:"Trådlöst mikrofonset, 8 kanaler",price:2999},{id:"mikrofonstativ",name:"Mikrofonstativ",price:60}],"Stativ & kablar":[{id:"h-gtalarstativ-gravity",name:"Högtalarstativ Gravity",price:40},{id:"h-gtalarstativ-k-m",name:"Högtalarstativ K&M",price:50},{id:"h-gtalarstativ-gravity-premium",name:"Högtalarstativ Gravity Premium",price:60},{id:"h-gtalarstativ-gravity-med-vev",name:"Högtalarstativ med vev",price:120},{id:"xlr-kablar",name:"XLR kablar",price:40},{id:"bluetooth-mottagare",name:"Bluetooth-mottagare",price:80}]}},Ljus:{sub:{"Färdiga paket":[{id:"ljus-small",name:"Ljuspaket, Small",price:399},{id:"ljus-small-plus",name:"Ljuspaket, Small+",price:599},{id:"ljus-small-pp",name:"Ljuspaket, Small++",price:799},{id:"ljus-medium",name:"Ljuspaket, Medium",price:1199},{id:"ljus-medium-plus",name:"Ljuspaket, Medium+",price:1299},{id:"ljus-medium-pp",name:"Ljuspaket, Medium++",price:1499},{id:"ljus-large",name:"Ljuspaket, Large",price:1699},{id:"ljus-large-plus",name:"Ljuspaket, Large+",price:2199},{id:"ljus-large-pp",name:"Ljuspaket, Large++",price:2699}],"Lösa effekter":[{id:"led-par",name:"LED PAR (14x8W RGBW)",price:79},{id:"led-par-9",name:"LED PAR (9x10W RGBW)",price:99},{id:"led-par-uv",name:"LED PAR UV",price:119},{id:"led-par-xl",name:"LED PAR XL (60x2W RGBW)",price:149},{id:"led-uppladdningsbar",name:"LED-armatur, uppladdningsbar",price:199}],"Rök & pyro":[{id:"rokmaskin-1500",name:"Rökmaskin (1500W)",price:349},{id:"rokmaskin-650",name:"Rökmaskin (650W)",price:449},{id:"kallgnist",name:"Kallgnistmaskin",price:499},{id:"konfetti",name:"Konfettiavfyrare",price:499}],"Stativ & tross":[{id:"stativ-400",name:"Stativ med T-bar (400 mm)",price:40},{id:"stativ-1200",name:"Stativ med T-bar (1200 mm)",price:40},{id:"stativ-900p",name:"Stativ med T-bar (900 mm, premium)",price:60},{id:"vevstativ-1200",name:"Vevstativ med T-bar (1200 mm)",price:120},{id:"tross-1m",name:"Trosstorn 1,0 m",price:300},{id:"tross-15m",name:"Trosstorn 1,5 m",price:360}]}},"Projektor & skärm":{products:[{id:"projektor-xga",name:"Projektor (XGA)",price:299},{id:"projektor-fhd",name:"Projektor (Full HD)",price:499},{id:"skarm-65",name:'Skärm 65"',price:399},{id:"skarm-75",name:'Skärm 75"',price:799},{id:"trailer-skarm",name:"Trailerupphängd skärm",price:1999}]},"Projektor tillbehör":{products:[{id:"projektorbord",name:"Projektorbord",price:160},{id:"projektorduk-100-",name:'Projektorduk 100"',price:400},{id:"golvstativ-65-75-",name:'Golvstativ 65"/75"',price:240},{id:"hdmi-kabel-5-m",name:"HDMI-kabel 5 m",price:48},{id:"hdmi-kabel-10-m",name:"HDMI-kabel 10 m",price:60},{id:"hdmi-splitter-4-port-",name:"HDMI-splitter (4-port)",price:400},{id:"mediaspelare-4k",name:"Mediaspelare 4K",price:400},{id:"scenmonitor-23-",name:'Scenmonitor 23"',price:480},{id:"usb-clicker-",name:"USB Clicker",price:60}]},"DJ-utrustning":{products:[{id:"dj-controller-numark",name:"DJ-controller Numark Mixstream Pro+",price:799},{id:"dj-controller-denon-go",name:"DJ-controller Denon Prime GO+",price:999},{id:"dj-controller-denon-prime4",name:"DJ-controller Denon Prime 4+",price:1499},{id:"dj-rane-system-one",name:"DJ-system Rane System One",price:1999},{id:"dj-bord",name:"DJ-bord",price:80},{id:"dj-bord2",name:"DJ-bord med paneler",price:200},{id:"scenengineer",name:"Hyr Scenengineer",price:952},{id:"media-editor",name:"Hyr Media-editor",price:952}]},"El & kabel":{products:[{id:"grenuttag-1-5-m",name:"Grenuttag 1,5 m",price:20},{id:"f-rl-ngningskabel-10-m",name:"Förlängningskabel 10 m",price:48},{id:"grenuttag-p-rulle-10-m",name:"Grenuttag på rulle 10 m",price:100},{id:"grenuttag-p-rulle-25-m",name:"Grenuttag på rulle 25 m",price:159}]},Tillägg:{products:[{id:"lev-standard",name:"Leverans & upphämtning (standard bil)",price:1118},{id:"lev-skrymmande",name:"Leverans & upphämtning (bil med släp)",price:1598},{id:"lev-lastbil",name:"Leverans & upphämtning (lätt lastbil)",price:2198},{id:"lev-bakgavel",name:"Leverans & upphämtning (lastbil med bakgavellift)",price:2998},{id:"rigg-teknik",name:"Teknikertjänst på plats (per tim)",price:600},{id:"montering",name:"Montering & demontering (per tim)",price:600},{id:"faktura",name:"Fakturaavgift",price:49}]},"Egen rad":{products:[{id:"custom",name:"Ange benämning och pris →",price:0,custom:!0}]}};n("new-quote-btn").addEventListener("click",()=>G(null));n("quote-close").addEventListener("click",F);n("quote-overlay").addEventListener("click",e=>{e.target===n("quote-overlay")&&F()});let p=[];function G(e){if(p=[],n("qc-name").value=e?.customer_name||"",n("qc-email").value=e?.customer_email||"",n("qc-phone").value=e?.customer_phone||"",n("qc-date").value=e?.event_date||"",n("qc-location").value=e?.event_location||"",n("qc-note").value="",n("quote-error").style.display="none",n("quote-send-btn").disabled=!1,n("quote-send-btn").textContent="✉ Skicka offert",e?.items?.length){e.items.filter(r=>!r._note).forEach(r=>{p.push({id:r.id||"",name:r.name,price:r.price||0,qty:r.qty||1})});const t=e.items.find(r=>r._note);t&&(n("qc-note").value=t.name)}const a=n("qp-cat");a.innerHTML='<option value="">— välj kategori —</option>'+Object.keys(A).map(t=>`<option value="${t}">${t}</option>`).join(""),n("qp-sub-wrap").style.display="none",n("qp-prod").innerHTML='<option value="">— välj kategori —</option>',_(),n("quote-overlay").classList.add("open"),n("qc-name").focus()}function U(e){G(e)}function F(){n("quote-overlay").classList.remove("open")}function Se(){const e=n("qp-cat").value,a=A[e],t=n("qp-sub-wrap"),r=n("qp-prod");if(!a){t.style.display="none",r.innerHTML='<option value="">— välj kategori —</option>';return}if(a.sub){const i=n("qp-sub");i.innerHTML='<option value="">— välj underkategori —</option>'+Object.keys(a.sub).map(o=>`<option value="${o}">${o}</option>`).join(""),t.style.display="",r.innerHTML='<option value="">— välj underkategori —</option>'}else t.style.display="none",re(a.products)}function _e(){const e=n("qp-cat").value,a=n("qp-sub").value,t=A[e];!t?.sub||!a||re(t.sub[a]||[])}function re(e){const a=n("qp-prod");a.innerHTML='<option value="">— välj produkt —</option>'+e.map(t=>{const r=t.custom?"✏ Skriv in benämning och pris själv":`${t.name} — ${t.price.toLocaleString("sv")} kr`;return`<option value="${t.id}" data-price="${t.price}" data-name="${t.name.replace(/"/g,"&quot;")}" data-custom="${t.custom||!1}">${r}</option>`}).join("")}function qe(){const e=n("qp-prod"),a=e.options[e.selectedIndex];if(!a||!a.value)return;if(a.dataset.custom==="true"){p.push({id:"custom-"+Date.now(),name:"",price:0,qty:1,custom:!0}),_(),setTimeout(()=>{const c=document.querySelectorAll(".quote-custom-name");c.length&&c[c.length-1].focus()},50);return}const r=a.dataset.name||a.textContent.split(" — ")[0],i=parseInt(a.dataset.price)||0,o=a.value,d=p.find(c=>c.id===o&&!c.custom);d?d.qty++:p.push({id:o,name:r,price:i,qty:1}),_(),e.selectedIndex=0}function _(){const e=n("quote-prod-tbody"),a=n("quote-prod-tfoot");if(!e)return;e.innerHTML=p.map((r,i)=>{const o=r.custom?`<input class="prod-input quote-custom-name" value="${r.name.replace(/"/g,"&quot;")}" placeholder="Ange benämning..." onchange="quoteEditName(${i},this.value)"
    ...
    </tr>`}).join("");const t=p.reduce((r,i)=>r+i.price*i.qty,0);a.innerHTML=p.length?`<tr><td colspan="3" style="font-weight:700;padding-top:0.5rem;font-size:0.82rem">Totalt (exkl. moms)</td>
       <td style="text-align:right;font-weight:700;color:var(--brand-lav);font-size:0.82rem">${u(t)} kr</td><td></td></tr>
       <tr><td colspan="3" style="color:var(--text-faint);font-size:0.72rem">Inkl. 25% moms</td>
       <td style="text-align:right;color:var(--text-muted);font-size:0.72rem">${u(Math.round(t*1.25))} kr</td><td></td></tr>`:""}function Le(e,a){p[e]&&(p[e].qty=Math.max(1,p[e].qty+a),_())}function Ee(e,a){p[e]&&(p[e].name=a)}function Me(e,a){p[e]&&(p[e].price=parseFloat(a)||0,_())}function Te(e){p.splice(e,1),_()}async function je(){const e=n("qc-name").value.trim(),a=n("qc-email").value.trim(),t=n("quote-error");if(t.style.display="none",!e){t.textContent="Namn krävs.",t.style.display="block",n("qc-name").focus();return}if(!a||!a.includes("@")){t.textContent="Giltig e-post krävs.",t.style.display="block",n("qc-email").focus();return}if(p.length===0){t.textContent="Lägg till minst en produkt.",t.style.display="block";return}const r=n("quote-send-btn");r.disabled=!0,r.textContent="⏳ Skickar…";const i={customer:{name:e,email:a,phone:n("qc-phone").value.trim()||null,date:n("qc-date").value||null,location:n("qc-location").value.trim()||null},items:p,note:n("qc-note").value.trim()||null};try{const{ok:o,data:d}=await f("admin-send-quote",{method:"POST",body:JSON.stringify(i)});o?(F(),l(`Offert skickad till ${a}! 🎉`),b()):(t.textContent=d?.error||"Kunde inte skicka offert.",t.style.display="block",r.disabled=!1,r.textContent="✉ Skicka offert")}catch{t.textContent="Nätverksfel — försök igen.",t.style.display="block",r.disabled=!1,r.textContent="✉ Skicka offert"}}n("panel-close").addEventListener("click",q);n("detail-overlay").addEventListener("click",q);document.addEventListener("keydown",e=>e.key==="Escape"&&q());function q(){n("detail-overlay").classList.remove("open"),n("detail-panel").classList.remove("open"),s=null}n("cal-prev").addEventListener("click",()=>{--k<0&&(k=11,h--),z()});n("cal-next").addEventListener("click",()=>{++k>11&&(k=0,h++),z()});n("cal-today").addEventListener("click",()=>{h=new Date().getFullYear(),k=new Date().getMonth(),z()});function z(){const e=["Januari","Februari","Mars","April","Maj","Juni","Juli","Augusti","September","Oktober","November","December"];n("cal-month-label").textContent=`${e[k]} ${h}`;const a=new Date(h,k,1),t=new Date(h,k+1,0),r=new Date;let i=a.getDay()-1;i<0&&(i=6);const o=n("cal-days");o.innerHTML="";const d=Math.ceil((i+t.getDate())/7)*7;for(let c=0;c<d;c++){const v=c-i+1,y=new Date(h,k,v),D=v<1||v>t.getDate(),B=!D&&y.toDateString()===r.toDateString(),E=`${y.getFullYear()}-${String(y.getMonth()+1).padStart(2,"0")}-${String(y.getDate()).padStart(2,"0")}`,w=K.filter(g=>g.event_date===E&&g.status!=="cancelled"),$=document.createElement("div");$.className=`cal-day${D?" other-month":""}${B?" today":""}`,$.innerHTML=`<div class="cal-day-num">${y.getDate()}</div>
      ${w.map(g=>`
        <div class="cal-event s-${g.status}" data-id="${g.id}">
          ${j[g.status]?.icon} ${g.customer_name||g.id}
        </div>`).join("")}`,$.querySelectorAll(".cal-event").forEach(g=>{g.addEventListener("click",ne=>{ne.stopPropagation(),R(g.dataset.id)})}),o.appendChild($)}}n("refresh-btn").addEventListener("click",()=>{b(),l("Uppdaterat!")});function Pe(){clearInterval(X),X=setInterval(b,3e4)}ie();window.prodQty=se;window.prodDelete=le;window.prodAddRow=ce;window.prodSave=ee;window.renderTab=m;window.prodTogglePopout=ve;window.prodAddFromCatalog=ge;window.prodAddFreeRow=be;window.prodUpdateSvc=ye;window.prodSaveAndSendOffer=ke;window.invMarkPaid=ae;window.invSendEmail=$e;window.invSaveFields=te;window.openPanel=R;window.closePanel=q;window.sendMessage=H;window.confirmOrder=O;window.cancelOrder=M;window.notifyCustomer=C;window.quotePickerCat=Se;window.quotePickerSub=_e;window.quotePickerAdd=qe;window.quoteQtyIdx=Le;window.quoteEditName=Ee;window.quoteEditPrice=Me;window.quoteDelIdx=Te;window.sendQuote=je;window.closeQuoteModal=F;window.openQuoteModal=G;window.openQuoteModalForCart=U;
