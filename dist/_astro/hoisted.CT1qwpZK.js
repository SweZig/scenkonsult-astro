let C="",J=[],o=null,S=[],H=[],M="info",h=new Date().getFullYear(),k=new Date().getMonth(),W=null;const n=e=>document.getElementById(e),m=e=>typeof e=="number"?e.toLocaleString("sv-SE"):"–",V=e=>e?new Date(e).toLocaleDateString("sv-SE"):"–",x=e=>e?new Date(e).toLocaleString("sv-SE",{dateStyle:"short",timeStyle:"short",hour12:!1}):"–";function c(e,a="success"){const t=n("toast");t.textContent=(a==="success"?"✓ ":"✕ ")+e,t.className=`show ${a}`,clearTimeout(t._t),t._t=setTimeout(()=>t.className="",3e3)}const j={open:{icon:"📬",label:"Inkommen",cls:"s-open"},proposal:{icon:"📤",label:"Offert skickad",cls:"s-proposal"},waiting:{icon:"⏳",label:"Väntar svar",cls:"s-waiting"},confirmed:{icon:"✅",label:"Bekräftad",cls:"s-confirmed"},fakturerad:{icon:"🧾",label:"Fakturerad",cls:"s-fakturerad"},betald:{icon:"💚",label:"Betald",cls:"s-betald"},cancelled:{icon:"❌",label:"Avbruten",cls:"s-cancelled"}};function de(){let e=sessionStorage.getItem("sk_admin_token");if(!e)try{const a=JSON.parse(localStorage.getItem("sk_admin_mode")||"{}"),t=8*60*60*1e3;a.token&&a.ts&&Date.now()-a.ts<t&&(e=a.token,sessionStorage.setItem("sk_admin_token",e))}catch{}e&&(C=e,Z())}n("login-pw").addEventListener("keydown",e=>e.key==="Enter"&&Y());n("login-btn").addEventListener("click",Y);function Y(){const e=n("login-pw").value.trim();e&&(C=e,sessionStorage.setItem("sk_admin_token",e),localStorage.setItem("sk_admin_mode",JSON.stringify({ts:Date.now(),token:e})),Z())}function Z(){n("login-screen").style.display="none",n("app").style.display="flex",b(),De()}n("logout-btn").addEventListener("click",()=>{sessionStorage.removeItem("sk_admin_token"),localStorage.removeItem("sk_admin_mode"),location.reload()});document.querySelectorAll(".nav-btn").forEach(e=>{e.addEventListener("click",()=>{document.querySelectorAll(".nav-btn").forEach(t=>t.classList.remove("active")),document.querySelectorAll(".view").forEach(t=>t.classList.remove("active")),e.classList.add("active");const a=e.dataset.view;n(`view-${a}`).classList.add("active"),a==="calendar"&&z()})});async function g(e,a={}){const t=await fetch(`/.netlify/functions/${e}`,{...a,headers:{"Content-Type":"application/json",Authorization:`Bearer ${C}`,...a.headers||{}}}),r=await t.json();if(t.status===401)throw sessionStorage.removeItem("sk_admin_token"),C="",n("app").style.display="none",n("login-screen").style.display="flex",n("login-error").style.display="block",n("login-error").textContent="Fel lösenord, försök igen.",n("login-pw").value="",n("login-pw").focus(),new Error("Unauthorized");return{ok:t.ok,status:t.status,data:r}}async function b(){const{ok:e,data:a}=await g("admin-carts");if(!e){if(a.error==="Ej behörig"){n("login-error").style.display="block",n("login-error").textContent="Fel lösenord.",n("app").style.display="none",n("login-screen").style.display="flex",sessionStorage.removeItem("sk_admin_token");return}c("Kunde inte hämta varukorgar","error");return}J=a.carts||[];const t=a.summary?.total_unread||0,r=n("global-unread");t>0?(r.style.display="inline",r.textContent=`${t} nya`):r.style.display="none",R(),n("view-calendar").classList.contains("active")&&z()}function se(){const e=n("filter-search").value.toLowerCase().trim(),a=n("filter-from").value,t=n("filter-to").value;return J.filter(r=>!(e&&!((r.customer_name||"").toLowerCase().includes(e)||(r.id||"").toLowerCase().includes(e))||a&&r.event_date&&r.event_date<a||t&&r.event_date&&r.event_date>t))}["filter-search","filter-from","filter-to"].forEach(e=>{n(e).addEventListener("input",R)});function R(){const e=se(),a=["open","waiting","confirmed","fakturerad","betald","cancelled"],t=n("kanban-board");t.innerHTML=a.map(r=>{const i=j[r],s=e.filter(l=>r==="waiting"?l.status==="waiting"||l.status==="proposal":l.status===r);return`
    ...
    </div>`}async function Q(e){o=null,S=[],H=[],M="info";const a=n("detail-overlay"),t=n("detail-panel");a.classList.add("open"),t.classList.add("open"),n("panel-body").innerHTML='<div class="loading-center"><div class="spinner"></div></div>',n("panel-footer").innerHTML="",n("panel-title").textContent="Laddar…";let r,i;try{({ok:r,data:i}=await g(`admin-carts?id=${e}`))}catch(p){c("Kunde inte ladda varukorg: "+p.message,"error"),q();return}if(!r){c("Kunde inte ladda varukorg","error"),q();return}o=i.cart,S=i.messages||[],H=i.audit_log||[],n("panel-title").textContent=`${o.customer_name||"Okänd"} — ${o.id}`;const s=document.getElementById("panel-quick");s&&s.remove();const l=S.filter(p=>p.sender==="customer"&&!p.read_at).length,d=n("chat-unread-badge");l>0?(d.style.display="inline",d.textContent=l):d.style.display="none",ee(),f("info"),L()}function ee(){document.querySelectorAll(".panel-tab").forEach(e=>{e.classList.toggle("active",e.dataset.tab===M),e.onclick=()=>{M=e.dataset.tab,ee(),f(M)}})}function f(e){const a=n("panel-body");e==="info"&&(a.innerHTML=me()),e==="products"&&(a.innerHTML=ge()),e==="faktura"&&(a.innerHTML=Se()),requestAnimationFrame(()=>{document.querySelectorAll(".status-select, .admin-note-area, .prod-input, .chat-textarea").forEach(t=>{t.style.setProperty("background","#221f50","important"),t.style.setProperty("color","#f0eeff","important"),t.style.setProperty("color-scheme","dark","important"),(t.tagName==="SELECT"||t.classList.contains("admin-note-area"))&&t.style.setProperty("border","1px solid rgba(196,181,244,0.25)","important")})}),e==="chat"&&we(),e==="log"&&(a.innerHTML=$e()),e==="info"&&(n("status-select")?.addEventListener("change",async t=>{const r=t.target.value;if(!confirm(`Ändra status till "${j[r]?.label}"?`)){t.target.value=o.status;return}const{ok:i}=await g("cart-update",{method:"POST",body:JSON.stringify({cart_id:o.id,status:r})});i?(o.status=r,c("Status uppdaterad!"),b(),L()):c("Fel vid statusändring","error")}),document.querySelectorAll('input[name="ctype"]').forEach(t=>{t.addEventListener("change",()=>{const r=document.querySelector('input[name="ctype"]:checked')?.value==="b2b",i=document.getElementById("b2b-fields");i&&(i.style.display=r?"grid":"none")})}),n("save-info-btn")?.addEventListener("click",async()=>{const t=n("save-info-btn");t.disabled=!0,t.textContent="⏳ Sparar…";const r={cart_id:o.id,customer_name:n("edit-name")?.value.trim()||null,customer_email:n("edit-email")?.value.trim()||null,customer_phone:n("edit-phone")?.value.trim()||null,event_date:n("edit-date")?.value||null,event_location:n("edit-location")?.value.trim()||null,customer_type:document.querySelector('input[name="ctype"]:checked')?.value||"b2c",customer_address:n("edit-address")?.value.trim()||null,customer_orgnr:n("edit-orgnr")?.value.trim()||null,customer_ref:n("edit-custref")?.value.trim()||null},{ok:i}=await g("cart-update",{method:"POST",body:JSON.stringify(r)});i?(Object.assign(o,{customer_name:r.customer_name,customer_email:r.customer_email,customer_phone:r.customer_phone,event_date:r.event_date,event_location:r.event_location,customer_type:r.customer_type,customer_address:r.customer_address,customer_orgnr:r.customer_orgnr,customer_ref:r.customer_ref}),n("panel-title").textContent=`${o.customer_name||"Okänd"} — ${o.id}`,c("Kunduppgifter sparade!"),b()):c("Kunde inte spara","error"),t.disabled=!1,t.textContent="💾 Spara kunduppgifter"}),n("save-notes-btn")?.addEventListener("click",async()=>{const t=n("admin-notes").value,{ok:r}=await g("cart-update",{method:"POST",body:JSON.stringify({cart_id:o.id,notes_admin:t})});r?(o.notes_admin=t,c("Anteckning sparad!")):c("Kunde inte spara","error")})),e==="chat"&&(n("chat-send-btn")?.addEventListener("click",K),n("chat-input")?.addEventListener("keydown",t=>{t.key==="Enter"&&(t.ctrlKey||t.metaKey)&&K()}))}function P(){const e=document.querySelectorAll("#prod-tbody .prod-row");return Array.from(e).map(a=>{const t=s=>a.querySelector(`[data-field="${s}"]`)?.value?.trim()||"",r=parseInt(t("qty"))||1,i=parseFloat(t("price"))||0;return{id:t("id"),name:t("name"),category:t("category"),qty:r,price:i}}).filter(a=>a.name)}function G(e){const a=document.querySelector(`[data-idx="${e}"]`);if(!a)return;const t=parseInt(a.querySelector('[data-field="qty"]')?.value)||1,r=parseFloat(a.querySelector('[data-field="price"]')?.value)||0,i=document.getElementById(`prod-sum-${e}`);i&&(i.textContent=m(t*r));const l=P().reduce((p,y)=>p+y.price*y.qty,0),d=document.getElementById("prod-tfoot");d&&(d.innerHTML=`<tr class="total-row"><td colspan="3">Totalt (exkl. moms)</td>
  ...
  `}function fe(){let e='<option value="">+ Välj produkt att lägga till</option>';return Object.entries(I).forEach(([a,t])=>{t.products?(e+=`<optgroup label="${a}">`,t.products.forEach(r=>{const i=JSON.stringify({id:r.id,name:r.name,price:r.price,category:a}).replace(/"/g,"&quot;");e+=`<option value="${i}">${r.name} — ${m(r.price)} kr</option>`}),e+="</optgroup>"):t.sub&&Object.entries(t.sub).forEach(([r,i])=>{e+=`<optgroup label="${a} — ${r}">`,i.forEach(s=>{const l=JSON.stringify({id:s.id,name:s.name,price:s.price,category:`${a} — ${r}`}).replace(/"/g,"&quot;");e+=`<option value="${l}">${s.name} — ${m(s.price)} kr</option>`}),e+="</optgroup>"})}),e}function ve(e){const a=(e||"").toLowerCase();return a.includes("scen")?{icon:"🎪",color:"var(--blue)"}:a.includes("ljud")?{icon:"🔊",color:"var(--green)"}:a.includes("ljus")?{icon:"💡",color:"var(--amber)"}:a.includes("dj")?{icon:"🎧",color:"var(--brand-lav)"}:a.includes("bild")?{icon:"📽",color:"var(--blue)"}:a.includes("tjänst")||a.includes("leverans")||a.includes("montering")?{icon:"🚚",color:"var(--gray)"}:{icon:"📦",color:"var(--gray)"}}function ge(){const e=o.items||[],a=e.find(d=>d._note),t=e.filter(d=>!d._note),r=t.reduce((d,p)=>d+(p.price||0)*(p.qty||p.quantity||1),0),i=["Tjänster","Tillägg"],s=t.findIndex(d=>i.includes(d.category)),l=t.map((d,p)=>{const y=d.qty||d.quantity||1,E=d.price||0,F=y*E,{icon:N,color:B}=ve(d.category);i.includes(d.category);const w=p===s&&s>0?'<tr><td colspan="5" style="padding:4px 0 2px"><div style="border-top:1px dashed rgba(196,181,244,0.25);margin:2px 0"></div><div style="font-size:0.7rem;text-transform:uppercase;letter-spacing:0.08em;color:rgba(196,181,244,0.4);padding:2px 4px">Tjänster</div></td></tr>':"",v=d.id||"",$=typeof PROD_DESC<"u"&&PROD_DESC[v]?PROD_DESC[v]:null,ie=$?`<div style="font-size:0.78rem;color:var(--text-muted);line-height:1.5;margin-top:4px">${$.desc||""}</div>${$.includes?.length?'<ul style="font-size:0.72rem;color:var(--text-faint);padding-left:14px;margin-top:3px">'+$.includes.map(oe=>`<li>${oe}</li>`).join("")+"</ul>":""}`:`<div style="font-size:0.78rem;color:var(--text-faint);margin-top:4px">Artno: ${v||"–"} · Kategori: ${d.category||"–"}</div>`;return w+`<tr data-idx="${p}" class="prod-row">
      <td style="vertical-align:top;padding:8px 6px">
        <div style="display:flex;align-items:flex-start;gap:6px">
          <button onclick="prodTogglePopout(${p})" style="flex-shrink:0;width:28px;height:28px;border-radius:6px;border:1px solid var(--border);background:rgba(0,0,0,.2);color:${B};cursor:pointer;font-size:14px;display:flex;align-items:center;justify-content:center" title="Produktinfo">${N}</button>
      ...
      <td style="text-align:right;vertical-align:top;padding-top:10px;font-weight:600;color:var(--brand-lav);font-size:0.85rem" id="prod-sum-${p}">${m(F)} kr</td>
  ...
  `}function ye(e){const a=document.getElementById("po-"+e);a&&(a.style.display=a.style.display==="none"?"block":"none")}function be(e){if(e.value)try{const a=JSON.parse(e.value.replace(/&quot;/g,'"')),t=P();t.push({id:a.id||"",name:a.name||"",price:a.price||0,qty:1,category:a.category||""}),o.items=t,f("products")}catch{c("Fel vid tillägg","error")}}function ke(){const e=P();e.push({id:"",name:"",price:0,qty:1,category:""}),o.items=e,f("products")}function he(){const e=P().filter(t=>!["lev-standard","lev-skrymmande","lev-lastbil","lev-standard-e","montering","fakturaavgift","lev-standard-e","lev-skrymmande-e","lev-lastbil-e"].some(r=>t.id===r||t.id===r+"-e")),a=t=>{const r=document.getElementById(t);if(!r||!r.value)return;const[i,s,l]=r.value.split("|");i&&s&&e.push({id:l,name:i,price:parseInt(s),qty:1,category:"Tjänster"})};a("svc-leverans"),a("svc-montering"),a("svc-faktura"),o.items=e,f("products")}async function xe(){await te(),X(o)}function we(){const e=n("panel-body"),a=S,t=a.length===0?'<div class="chat-empty">Ingen chatt ännu.</div>':`<div class="chat-messages">${a.map(r=>{const i=r.sender==="admin"?r.read_at?`<span class="read-check">✓✓</span> Läst ${x(r.read_at)}`:"⏳ Inte läst":r.read_at?`Läst ${x(r.read_at)}`:"";return`
    ...
    </div>`,setTimeout(()=>{const r=e.querySelector(".chat-messages");r&&(r.scrollTop=r.scrollHeight),e.scrollTop=e.scrollHeight},50)}async function K(){const e=n("chat-input"),a=n("chat-send-btn"),t=e?.value.trim();if(!t)return;a.disabled=!0,a.textContent="Skickar…";const{ok:r,data:i}=await g("cart-message",{method:"POST",body:JSON.stringify({cart_id:o.id,body:t})});r?(S.push({id:i.message?.id||Date.now(),cart_id:o.id,sender:"admin",body:t,read_at:null,created_at:new Date().toISOString()}),e.value="",f("chat"),c("Meddelande skickat!"),b()):(c(i.error||"Kunde inte skicka","error"),a.disabled=!1,a.textContent="Skicka till kund →")}function $e(){const e=H;return e.length===0?'<div class="empty-col" style="padding:3rem">Ingen logg ännu.</div>':`
