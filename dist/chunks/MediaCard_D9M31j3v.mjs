...
</span> <div class="text-gray-400 text-[10px] mt-0.5">${priceSuffix}</div> </div> ${renderComponent($$result, "CartButton", $$CartButton, { "id": cartId, "name": title, "price": priceExcl, "priceNote": `/${priceSuffix.replace(/^\//, "")}`, "artno": artno ?? "", "category": category, "image": images[0], "size": "sm", "label": "L\xE4gg i varukorg" })} </div> </div> </article>  `;
