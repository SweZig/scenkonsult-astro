...
const VILLKOR_B2C = [
  ...
  ['2. Bokning och betalning',
   'Privatpersoner betalar alltid innan eller i samband med att utrustningen lämnas ut — via Swish, förskottsfaktura eller kortbetalning vid hämtning. Kortbetalning vid hämtning sker via SumUp och vi tar de flesta betalkort (Visa, Mastercard, Maestro, American Express) samt Apple Pay och Google Pay. Bokningar inom 72 timmar kräver omedelbar betalning. Bokningsavgift om 49 kr (exkl. moms) tillkommer.'],
  ...
  ['7. Montering och teknik',
   'Enklare utrustning levereras för självmontering. Scenpaket Large och uppåt kräver professionell montering (prissätts separat). LED-skärmar och komplex ljusutrustning kräver tekniker. Monteringstjänst debiteras per påbörjad 15-minutersperiod à 150 kr exkl. moms (600 kr/tim).'],
...
const VILLKOR_B2B = [
  ['1. Hyresperiod',
   'Normal hyresperiod är 22 timmar — hämtning kl 13:00 och återlämning kl 11:00 påföljande dag. Längre hyresperioder mot tillägg. Utrustning som inte återlämnas i tid debiteras extra hyresdag per påbörjat dygn.'],
  ['2. Bokning och betalning',
   'Bokning är bindande och bekräftas skriftligen. Betalning sker normalt mot faktura (vanligen 5–30 dagar). Företag utan kreditmöjlighet kan i stället betala vid hämtning med kort via SumUp (de flesta betalkort accepteras, samt Apple Pay och Google Pay) eller via Swish. Bokningar inom 72 timmar kräver omedelbar betalning. Bokningsavgift om 49 kr (exkl. moms) tillkommer per faktura. Dröjsmålsränta debiteras vid sen betalning (referensränta + 8 %).'],
